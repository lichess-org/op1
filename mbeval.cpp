 ...
 * 3.1  Some fixes for parity handling; optionally compile without double pawn steps (20201107)
 * 3.2  Some more parity constrained endings (20210105)
 * 3.3  Better en passant handling (20210223)
 * 3.4  Fixed score output when parsing PGN files (20210313)
 * 3.5  Better display of bishop parities in PGN (20210316)
 * 3.6  Extract bad study moves directly from position file even if not merging with PGN (20210328)
 * 3.7  Note number of positions processed unless commenting games or finding best line (20210421)
 * 3.8  Slight output changes (20210509)
 ...
 * 4.3  Support reading database with 2 vs 1 pawn, with 1 opposing pair (20210910)
 * 5.0  Support YK files (20210925)
 * 5.1  Add high DTC for YK files; bug fix on YK <-> MB conversion (20211005)
 * 5.2  Ability to insert scores directly into PGN move list, including variations (20211110)
 * 5.3  Add 2 vs 2, with 1 opposing pair (20211111)
 * 5.4  Add 3 vs 1, and 1 vs 3 with 1 opposing pair (20211114)
 * 5.5  Bug fixes; add number of pieces to EGTB comments (20211130)
 * 5.6  Add max number of pieces to EGTB in annotator field (20211210)
 * 5.7  Bug fixes for number of pieces after a result changing capture; zz designation (20220109)
 * 5.8  Increased maximum line length in PGN to 8192; more tolerance for dangling symbols (20220115)
 * 5.9  Add 4 vs 1 and 1 vs 4 pawn configurations with 1 opposing pair (20220126)
 * 5.10 Add 3 vs 2 and 2 vs 3 pawn configurations with 1 opposing pair (20220126)
 * 5.11 Add 3 vs 3 configurations with 1 opposing pair (20220126)
 * 5.12 Add 4 vs 2 and 2 vs 4 configurations with 1 opposing pair (20220212)
 * 6.0  Add UCI wrapper (conditional compilation) (20220302)
 * 6.1  Point out errors in comments in annotator (20220501)
 * 6.2  Print out EPD for incorrectly played positions (20220508)
 * 6.3  Enforce number of pieces constraint for all PGN parsing (20220517)
 * 6.4  Handle long lines in PGN input for Chessbase (20230427)
 *      Optionally read MB or YK databases only (20230428)
 * 6.5  Fixed PGN output for long strings; correct assessment of final position (20230609)
 * 7.0  DP_22 configurations (two opposing pawn pairs, where the pawns can never capture
 *      each other) (20230707)
 * 7.1  Bug fix for large number of pieces (20230708)
 * 7.2  More info in verbose mode (20230711)
 * 7.3  Additional statistic output (20230718)
 * 7.4  Better handling of 9-man endings (20230805)
 * 7.5  Output adjustments for chessbase (20230903)
 * 7.6  In PGN output, optionally only include games with at least one TB evaluation (20231115)
 * 7.7  Optionally always tag positions with castling as unknown from EGTB (20240519)
 * 7.8  Some chessbase PGN file bug work-arounds (20240523)
 * 7.9  Tag endings where promotions are restricted (queen only, etc.) (20240529)
 */
...
enum {FREE_PAWNS = 0, BP_11_PAWNS, OP_11_PAWNS, OP_21_PAWNS, OP_12_PAWNS, OP_22_PAWNS, DP_22_PAWNS, 
	OP_31_PAWNS, OP_13_PAWNS, OP_41_PAWNS, OP_14_PAWNS, OP_32_PAWNS, OP_23_PAWNS, OP_33_PAWNS,
	OP_42_PAWNS, OP_24_PAWNS};

const char *OpExtensionName[] = {
     "Free Pawns", "BP_11", "OP_11", "OP_21", "OP_12", "OP_22", "DP_22",
     "OP_31", "OP_13", "OP_41", "OP_14", "OP_32", "OP_23", "OP_33",
     "OP_42", "OP_24"};
 ...
 * For a square board with opposite corners [0,0] and [NCOLS-1,NROWS-1], the single
 * king can be confined to the rectangle with corners:
 *
 ...
 *
 * which is 32 on an 8x8 board.  Special cases are 2x3 (2 columns, three rows), where 
 * the king can't be on a2, 3x2 (3 columns, two rows) where the king can't be on b1 or b2,
 * and 3x3, where the king can't be on b2.
 *
 * N_KINGS is the unique number in which two kings can be put on the board so that
 * they are not touching (1806 on an 8x8 board).
 */
...
#        define N_KINGS  (2*(NSQUARES-4) +\
			(NROWS-2)*(NSQUARES-6) +\
			(NSQUARES + NROWS - 8) +\
			(NROWS-2)*((NSQUARES + NROWS)/2 - 6))
#     endif
#  else
#     define N_SKINGS ((NROWS)*((NCOLS+1)/2))
#     define N_KINGS (2*(NSQUARES-4) +\
...
#  else
#     define N_SKINGS ((NROWS)*((NCOLS+1)/2))
#     define N_KINGS (4 + (NROWS + NCOLS - 2)*(NSQUARES - 6) +\
...
/*
 * Determine the number of distinct canonical positions of the two kings, given
 * the symmetry of pawnless pieces.  These depend on whether the board is a square
 * or a rectangle, and whether the sides are even or odd
 *
 * N_SKINGS_NOPAWNS is the unique number in which a single king can be put on the board.
 * For a square board with opposite corners [0,0] and [NCOLS-1,NCOLS-1], the single
 * king can be confined to the triangle with corners:
 *
 ...
 * be at the apex of the triangle b2, because the black king would have nowhere to go.
 * In that case, N_SKINGS_NOPAWNS = 2 (a1 and b1) rather than 3 as from the formula.
 *
 ...
 *
 * The only exception is for a 3x2 board, where the white king only has one unique square
 * instead of 2 from the formula.
 *
 * N_KINGS_NOPAWNS is the unique number in which two kings can be put on the board so that
 * they are not touching (462 on an 8x8 board). The formulas below are rather complicated,
 * taking into account that if the white king is on a symmetry axis, such as the a1-h8
 * diagonal on a square board, or the d3-d7 vertical on a 7x7 board, the black king can
 * be restricted to one of the sides that results from reflection on that symmetry axis.
 */
...
#    else
#      define N_KINGS_NOPAWNS (NCOLS*(NCOLS+1)+(NCOLS+1)*(NCOLS+3)/8-10 + \
		(NCOLS-3)/2*(2*NSQUARES+NCOLS+(NCOLS-5)/2*(NSQUARES-9)/2 - 18))
#      define N_SKINGS_NOPAWNS (((NCOLS+1)/2*((NCOLS+1)/2+1))/2)
#    endif
#  else			// square with even sides
#    define N_KINGS_NOPAWNS (NCOLS*(NCOLS+1)/2-3 + (NCOLS/2-1)*(NSQUARES-6) +\
		(NCOLS/2-1)*(NCOLS*(NCOLS+1)/2-6) + (NCOLS/2-2)*(NCOLS/2-1)/2*(NSQUARES-9))
#  endif
#  define N_SKINGS_NOPAWNS (((NCOLS+1)/2*((NCOLS+1)/2+1))/2)
...
#      define ODD_COLUMNS
#      define N_KINGS_NOPAWNS (NSQUARES-4 + (NCOLS-3)/2*(NSQUARES-6) + (NROWS-3)/2*(NSQUARES-6) + \
		NROWS*(NCOLS+1)/2-4 + NCOLS*(NROWS+1)/2-4 +\
                (NCOLS-3)/2*(NCOLS*(NROWS+1)/2-6) + (NROWS-3)/2*(NROWS*(NCOLS+1)/2-6) + \
		(NCOLS-3)/2*(NROWS-3)/2*(NSQUARES-9) +\
		(NCOLS+1)/2*(NROWS+1)/2-4)
#    else		// rows odd, columns even
#      define N_KINGS_NPAWNS ((NCOLS/2-1)*((NROWS-1)/2*NSQUARES+(NCOLS/2)*(NROWS+1)-9*(NROWS-3)/2-12)+\
		(NROWS-1)/2*NSQUARES+(NCOLS/2)*(NROWS+1)-3*NROWS+1)
#    endif
#  elif (NCOLS % 2)	// rows even, columns odd
#      define ODD_COLUMNS
#      define N_KINGS_NOPAWNS ((NROWS/2-1)*((NCOLS-1)/2*NSQUARES+(NROWS/2)*(NCOLS+1)-9*(NCOLS-3)/2-12)+\
		(NCOLS-1)/2*NSQUARES+(NROWS/2)*(NCOLS+1)-3*NCOLS+1)
#  else		// both sides even
#    define N_KINGS_NOPAWNS (NSQUARES-4 + (NROWS/2+NCOLS/2-2)*(NSQUARES-6) +\
...
#define N3_ODD_PARITY ((NUM_WHITE_SQUARES)*(NUM_BLACK_SQUARES)*(NUM_WHITE_SQUARES + NUM_BLACK_SQUARES - 2)/2)
#define N3_EVEN_PARITY ( (NUM_WHITE_SQUARES)*(NUM_WHITE_SQUARES-1)*(NUM_WHITE_SQUARES-2)/6 +\
			 (NUM_BLACK_SQUARES)*(NUM_BLACK_SQUARES-1)*(NUM_BLACK_SQUARES-2)/6 )
...
// compute N7 in such a way that for NSQUARES=64 no intermediate 32-bit overflow occurs
...
#define N2_1_OPPOSING ((NCOLS*(NCOLS-1)*(NROWS-1)*(NROWS-2)*(NROWS-3)/2) +\
	(2*(NCOLS-1)*(NROWS - 5 + 2)) +\
	(NCOLS*(NROWS-3)*(NROWS-2)*(2*NROWS - 5))/6)

#define N1_2_OPPOSING N2_1_OPPOSING

#define N4_Opposing_NoEP (NCOLS*(NCOLS-1)/2*(NROWS-2)*(NROWS-3)/2*(NROWS-2)*(NROWS-3)/2 +\
	(NCOLS*(NROWS-2)*(NROWS-3)*(NROWS-4)*(NROWS-5)/12))

#define N4_Opposing_EP (2*(NCOLS-1)*4*(NROWS - 5))

#define N4_ONE_COLUMN (NCOLS*(NROWS-2)*(NROWS-3)*(NROWS-4)*(NROWS-5)/12)
#define N4_NON_ADJACENT ((NCOLS-1)*(NCOLS-2)/2*(NROWS-2)*(NROWS-3)/2*(NROWS-2)*(NROWS-3)/2)
#define N4_ADJACENT (2*(NCOLS-1)*(NROWS-1)*(NROWS-2)*(NROWS-3)*(NROWS-4)/24)

#define N4_OPPOSING (N4_ONE_COLUMN + N4_NON_ADJACENT + N4_ADJACENT)

#define N2_2_Opposing_3 ((NROWS-2)*(NROWS-3)/2*NCOLS*((NROWS-1)*(NROWS-1)-1)*(NCOLS-1)*(NCOLS-2))
#define N2_2_Opposing_2a ((NROWS-2)*(NROWS-3)/2*(NROWS*(NROWS-1)/2 - 1)*NCOLS*(NCOLS-1))
#define N2_2_Opposing_2b (((NROWS-2)*(NROWS-3)*(NROWS-4)/6 + (NROWS-1)*(NROWS-2)*(NROWS-3)/6)*(NROWS-1) - (NROWS-2)*(NROWS-3)/2)*NCOLS*(NCOLS-1)
#define N2_2_Opposing_1 ((2*(NROWS-1)*(NROWS-2)*(NROWS-3)*(NROWS-4)/24 + NROWS*(NROWS-1)*(NROWS-2)*(NROWS-3)/24 - (NROWS-2)*(NROWS-3)/2)*NCOLS)

#define N2_2_Opposing_NoEP (N2_2_Opposing_3 + N2_2_Opposing_2a + 2*N2_2_Opposing_2b + N2_2_Opposing_1 + N4_Opposing_NoEP)
...
#define N2_2_Opposing_EP ((N2_2_EP_1 + N2_2_EP_2 + N2_2_EP_3 + N2_2_EP_4 + N2_2_EP_5 + N2_2_EP_6 + N2_2_EP_7)*2*2*(NCOLS-1) - \
...
#define N31_a2 ((NCOLS-1)*(NROWS-1) * ((NCOLS-1)*(NROWS-1) - 1) / 2 -\
	(NCOLS-1)*(NCOLS-2)/2)
...
#define N31_Opposing_NoEP ((N31_a1)*(N31_a2) + NCOLS*(NCOLS-1)*(N31_b1 + N31_b2) + NCOLS*(N31_c1 + N31_c2))
...
#define N31_ep_b (NCOLS-1)*2*(1 + 2*(2*(NROWS-5) + (NCOLS-2)*(NROWS-2)))
...
// For index calculations on 8x8 board, ensure zone size is divisible by NSQUARES
// for backwards compatibility with Yakov Konoval's index methodology.
...
// so that the zone size for 8 or 9-piece endings is always divisible by NSQUARES*NSQUARES
...
#define N2_ODD_PARITY_Offset (N2_ODD_PARITY + NSQUARES - (N2_ODD_PARITY % NSQUARES))
...
#define N2_EVEN_PARITY_Offset (N2_EVEN_PARITY + NSQUARES - (N2_EVEN_PARITY % NSQUARES))
...
#define N3_EVEN_PARITY_Offset (N3_EVEN_PARITY + NSQUARES - (N3_EVEN_PARITY % NSQUARES))
#else
...
static int
high_dtz_compar(const void *a, const void *b)
{
...
file f_open ( const char *szFile, const char *szMode )
{
	HANDLE  h;
	h = CreateFile (szFile,
			GENERIC_READ | ((('w' == szMode[0]) || ('w' == szMode[1])) ? GENERIC_WRITE : 0),
			(('r' == szMode[0]) && ('w' != szMode[1])) ? FILE_SHARE_READ : 0,
			NULL,
		      ('r' == szMode[0]) ? (('w' == szMode[1]) ? OPEN_ALWAYS : OPEN_EXISTING) : CREATE_ALWAYS,
...
size_t  f_read ( void *pv, size_t cb, file fp, INDEX indStart )
{
	...
	if(bResult == 0 && GetLastError() != NO_ERROR) {
		fprintf(stderr,"*** SetFilePointerEx failed in f_read: pos is %I64u code is %lx\n",
...
static void f_write ( void *pv, size_t cb, file fp, INDEX indStart )
{
	...
	bResult = SetFilePointerEx(fp, s, NULL, FILE_BEGIN);
	if(bResult == 0 && GetLastError() != NO_ERROR) {
		fprintf(stderr,"*** SetFilePointerEx failed in f_write: pos is %I64u code is %lx\n",
			indStart, GetLastError());
		exit(1);
	}
#else
	...
	if (0xFFFFFFFF == dwPtr && NO_ERROR != GetLastError()) {
		...
		printf("indStart " DEC_INDEX_FORMAT " indStart shifted 32 bits to the right %lx, lHiPos = %lx, low_pos = %ld\n",
	...
	if (0 == bResult || cbWritten != cb) {
		printf ("*** Write failed\n");
		printf("Tried to write " DEC_INDEX_FORMAT " actually wrote " DEC_INDEX_FORMAT "\n",
...
enum {NO_PIECE=0, PAWN = 1, KNIGHT=2, BISHOP=4, ARCHBISHOP=(KNIGHT|BISHOP), ROOK=8, CARDINAL=(KNIGHT|ROOK), QUEEN=(BISHOP|ROOK), MAHARAJA=(KNIGHT|BISHOP|ROOK), KING=16};

#define QRBN_PROMOTIONS ((1 << (KNIGHT)) | (1 << (BISHOP)) | (1 << (ROOK)) | (1 << (QUEEN)))
...
enum {LOST = 65000, DRAW, STALE_MATE, NOT_LOST, NOT_WON, HIGH_DTZ_MISSING, WON, 
      CHECK_MATE, ILLEGAL, UNRESOLVED, UNKNOWN,
      PLUS_MINUS, PLUS_EQUAL, EQUAL_MINUS, PLUS_EQUALMINUS, PLUSEQUAL_MINUS,
      MINUS_EQUAL, MINUS_PLUS, MINUS_PLUSEQUAL, EQUAL_PLUS, EQUALMINUS_PLUS, NO_MZUG};

enum {IDENTITY = 0, REFLECT_V, REFLECT_H, REFLECT_VH,\
      REFLECT_D, REFLECT_DV, REFLECT_DH, REFLECT_DVH};

enum {ETYPE_NOT_MAPPED = -65000, TOO_MANY_PIECES, MB_FILE_MISSING, YK_FILE_MISSING, BAD_ZONE_SIZE, BAD_ZONE_NUMBER,
      HEADER_READ_ERROR, OFFSET_ALLOC_ERROR, OFFSET_READ_ERROR, ZONE_ALLOC_ERROR,
      ZONE_READ_ERROR, BUF_ALLOC_ERROR, BUF_READ_ERROR};

enum {CAPTURE = 0x1, PROMOTION = 0x2, EN_PASSANT = 0x4, CHECK = 0x8, MATE = 0x10, 
      WK_CASTLE = 0x20, WQ_CASTLE = 0x40, BK_CASTLE = 0x80, BQ_CASTLE = 0x100,
      UNIQUE = 0x200, BEST = 0x400};
...
typedef enum { PAWN_MOVE, PAWN_MOVE_WITH_PROMOTION, ENPASSANT_PAWN_MOVE,
               PIECE_MOVE, KINGSIDE_CASTLE, QUEENSIDE_CASTLE, NULL_MOVE,
               UNKNOWN_MOVE
             } MoveClass;
...
typedef enum {
    ...
    STAR, EOS, OPERATOR, NO_TOKEN, ERROR_TOKEN
} PGNToken;
...
#define N2_Index_Function(a, b) ((a) > (b) ? ((a)*((a)-1)/2 + b) : ((b)*((b)-1)/2 + a))
...
static int N4_Index_Function(int a, int b, int c, int d)
{
    ...
    return a*(a-1)*(a-2)*(a-3)/24 + b*(b-1)*(b-2)/6 + c*(c-1)/2 + d;
}

#else	// !MB_INDEX (YK_INDEX)

#define N2_Index_Function(a, b) ((a) > (b) ? ((b)*((2*NSQUARES-3) - b)/2 + a - 1) : ((a)*((2*NSQUARES-3)-a)/2 + b - 1))
...
static int N3_Index_Function(int a, int b, int c)
{
    ...
    return c*(((3*NSQUARES*(NSQUARES-2)+2)-6*b) + c*(-3*(NSQUARES-1) + c))/6 +
...
static int N4_Index_Function(int a, int b, int c, int d)
{
    ...
    int b4 = d*((-6 + NSQUARES*(22 + NSQUARES*(-18 + 4*NSQUARES))) +
	      d*((-11 + NSQUARES*(18 - 6*NSQUARES)) + d*((4*NSQUARES-6) - d))) / 24;
    ...
    int b3 = c2*((3*(NSQUARES-d-1)*(NSQUARES-d-3)+2) + c2*((-3*(NSQUARES-d-2)) + c2)) / 6;
...
// For 5 or more identical pieces, always compute index rather than creating lookup table
...
static ZINDEX N5_Index_Function(int a, int b, int c, int d, int e)
{
    ...
    return k5_tab[a] + b*(b-1)*(b-2)*(b-3)/24 + c*(c-1)*(c-2)/6 + d*(d-1)/2 + e;
}
...
static ZINDEX N7_Index_Function(int a, int b, int c, int d, int e, int f, int g)
{
    if(b > a)
	SWAP(a, b);
    if(c > a)
...
#define N2_2_Index(a, b) (k2_same_color_tab[(a)/2 + (NUM_BLACK_SQUARES)*((b)/2)])
...
#define N3_Odd_Index(a, b, c) (k3_odd_tab[a + (NSQUARES)*(b + (NSQUARES)*(c))])
#define N3_Even_Index(a, b, c) (k3_even_tab[a + (NSQUARES)*(b + (NSQUARES*(c)))])
#define N2_Opposing_Index(a, b) (k2_opposing_tab[a + (NSQUARES)*(b)])
#define N4_Opposing_Index(a, b, c, d) \
	(k4_opposing_tab[Row(a) +(NROWS)*(Row(b) + (NROWS)*(c + (NSQUARES)*d))])
...
#define N2_2_Opposing_Index(a, b, c, d) \
	(k2_2_opposing_tab[a + (NSQUARES)*(b + (NSQUARES)*(c + (NSQUARES)*(d)))])
#endif	// 8x8 board
...
{
    Identity, ReflectV, ReflectH, ReflectVH,
    ReflectD, ReflectDV, ReflectDH, ReflectDVH
};

const char *SymmetryName[] = {"Identity", "Reflect_V", "Reflect_H", "Reflect_VH",
			"Reflect_D", "Reflect_DV", "Reflect_DH", "Reflect_DVH"
};

static int *InverseTransforms[] =
{
    Identity, ReflectV, ReflectH, ReflectVH,
    ReflectD, ReflectDH, ReflectDV, ReflectDVH
};

static int InverseTable[] = {
    IDENTITY, REFLECT_V, REFLECT_H, REFLECT_VH,
    REFLECT_D, REFLECT_DH, REFLECT_DV, REFLECT_DVH
};

static void GetFlipFunctionNoPawns(int wk, int bk, bool *flipped, int **transform)
{
    *flipped = false;
#if defined(SQUARE)
...
#define KK_Transform_NoPawns(wk, bk) KK_Transform_Table_NoPawns[((NSQUARES)*(wk) + (bk))]
#define KK_Index_NoPawns(wk, bk) KK_Index_Table_NoPawns[((NSQUARES)*(wk) + (bk))]
#else
...
static void InitN2Tables(int *tab, int *pos)
{
	...
	for(int p2 = p1; p2 < NSQUARES; p2++) {
	   ...
	   if(p1 == p2)
		...
		if(index != g_index) {
		    fprintf(stderr,"Bad pair index: index=%d computed index=%d p1=%d p2=%d\n", 
			index, g_index, p1, p2);
		    exit(1);
		}
		score = index++;
...
static void InitN2_2Tables(int *tab, int *pos)
{
	...
	for(int p1 = 0; p1 <= p2; p1++) {
		...
		if(index != g_index) {
		    fprintf(stderr,"Bad same color pair index=%d computed index=%d p1=%d p2=%d\n",
			index, g_index, p1, p2);
		    exit(1);
		}
		score = index++;
...
static void InitN2_1_OpposingTables(int *tab, int *pos)
{
	...
	for(int wp2 = wp1 + 1; wp2 < NSQUARES; wp2++) {
		...
		if(Row(wp1) == 0) {
		    ...
		    if((Column(wp1_physical) > 0 && bp1_physical == wp1_physical - 1) ||
			(Column(wp1_physical) < (NCOLS-1) && bp1_physical == wp1_physical + 1)) {
			ep_possible = true;
		    }
		    if(!ep_possible) {
			board[bp1_physical] = 0;
		...
		if(Row(bp1) == (NROWS-1)) {
		    ...
		    bool ep_possible = false;
		    if((Column(bp1_physical) > 0 && (wp1_physical == bp1_physical - 1 || wp2 == bp1_physical - 1)) ||
			(Column(bp1_physical) < (NCOLS-1) && (wp1_physical == bp1_physical + 1 || wp2 == bp1_physical + 1))) {
			ep_possible = true;
		    }
		    if(!ep_possible) {
			board[bp1_physical] = 0;
		...
		if((Column(wp1_physical) == Column(bp1_physical) && wp1_physical < bp1_physical) ||
		   (Column(wp2) == Column(bp1_physical) && wp2 < bp1_physical)) {
		    if(index >= N2_1_OPPOSING) {
			fprintf(stderr,"Two many 2 vs 1 pawn positions with an opposing pair\n");
			exit(1);
		    }
    ...
    if(index != N2_1_OPPOSING) {
	fprintf(stderr,"Saw %d 2 vs 1 pawns with one opposing pairs, expected %d\n",
	    index, N2_1_OPPOSING);
	exit(1);
    }
}
...
static void InitN1_2_OpposingTables(int *tab, int *pos)
{
	...
	for(int bp1 = 0; bp1 < (NSQUARES - NCOLS); bp1++) {
		...
		if(Row(wp1) == 0) {
		    ...
		    bool ep_possible = false;
		    if((Column(wp1_physical) > 0 && ((bp1 == wp1_physical - 1) || (bp2_physical == wp1_physical - 1))) ||
			(Column(wp1_physical) < (NCOLS-1) && ((bp1 == wp1_physical + 1) || (bp2_physical == wp1_physical + 1)))) {
			ep_possible = true;
		    }
		    if(!ep_possible) {
			board[bp2_physical] = 0;
		...
		if(Row(bp2) == (NROWS-1)) {
		    ...
		    bool ep_possible = false;
		    if((Column(bp2_physical) > 0 && (wp1_physical == bp2_physical - 1)) ||
			(Column(bp2_physical) < (NCOLS-1) && (wp1_physical == bp2_physical + 1))) {
			ep_possible = true;
		    }
		    if(!ep_possible) {
			board[bp2_physical] = 0;
		...
		// now check whether we have an opposing pair
		if((Column(wp1_physical) == Column(bp1) && wp1_physical < bp1) ||
		   (Column(wp1_physical) == Column(bp2_physical) && wp1_physical < bp2_physical)) {
		    if(index >= N1_2_OPPOSING) {
			fprintf(stderr,"Too many 1 vs 2 pawn positions with an opposing pair\n");
			exit(1);
		    }
    ...
    if(index != N1_2_OPPOSING) {
	fprintf(stderr,"Saw %d 1 vs 2 pawns with one opposing pairs, expected %d\n",
	    index, N1_2_OPPOSING);
	exit(1);
    }
}
...
static void InitN2_2_OpposingTables(int *tab, int *pos)
{
	...
	for(int wp2 = wp1 + 1; wp2 < NSQUARES; wp2++) {
		...
		for(int bp2 = bp1 + 1; bp2 < NSQUARES; bp2++) {
		    ...
		    // if there is a promoted pawn, the square before the promoted pawn must be empty
		    if(Row(wp2) == (NROWS-1)) {
			if(board[wp2 - NCOLS]) {
			    board[bp2_physical] = 0;
			    continue;
		    ...
		    int ep_square = -1;
		    // if there is a ep_square, check whether it is indeed a e.p. position
		    if(wp1 != wp1_physical) {
			ep_square = wp1_physical - NCOLS;
			if(board[ep_square] || board[ep_square - NCOLS]) {
			    board[bp2_physical] = 0;
			...
			bool ep_possible = false;
			if((Column(wp1_physical) > 0 && board[wp1_physical - 1] == -PAWN) ||
			   (Column(wp1_physical) < (NCOLS-1) && board[wp1_physical + 1] == -PAWN))
			    ep_possible = true;
			if(!ep_possible) {
			    board[bp2_physical] = 0;
			    continue;
			...
			bool ep_possible = false;
			if((Column(bp2_physical) > 0 && board[bp2_physical - 1] == PAWN) ||
			   (Column(bp2_physical) < (NCOLS-1) && board[bp2_physical + 1] == PAWN))
			    ep_possible = true;
			if(!ep_possible) {
			    board[bp2_physical] = 0;
			    continue;
		    ...
		    // now check whether there are is at least one opposing pawn pair
			...
			int paired = -1;
			// find the lowest available black pawn that can be paired
			for(int j = 0; j < num_black_pawns; j++) {
			    if(black_pawn_paired[j]) continue;
		    ...
		    pos[index] = bp2 + NSQUARES*(bp1 + NSQUARES * (wp2 + NSQUARES*wp1));
		    tab[bp2 + NSQUARES*(bp1 + NSQUARES*(wp2 + NSQUARES*wp1))] = index;
		    tab[bp1 + NSQUARES*(bp2 + NSQUARES*(wp2 + NSQUARES*wp1))] = index;
		    tab[bp2 + NSQUARES*(bp1 + NSQUARES*(wp1 + NSQUARES*wp2))] = index;
		    tab[bp1 + NSQUARES*(bp2 + NSQUARES*(wp1 + NSQUARES*wp2))] = index;
		    index++;
		    board[bp2_physical] = 0;
		}
		board[bp1] = 0;
	...
	fprintf(stderr,"Saw %d 2 vs 2 pawns with at least one opposing pair, expected %d\n",
	    index, N2_2_OPPOSING);
	exit(1);
    }
}
...
static void InitN3_1_OpposingTables(int *tab, int *pos)
{
	...
	for(int wp2 = wp1 + 1; wp2 < NSQUARES - NCOLS; wp2++) {
		...
		for(int bp1 = 2*NCOLS; bp1 < NSQUARES; bp1++) {
		    ...
		    if(board[bp1_physical]) continue;
		    board[bp1_physical] = -PAWN;
			...
			bool ep_possible = false;
			if((Column(bp1_physical) > 0 && board[bp1_physical - 1] == PAWN) ||
			   (Column(bp1_physical) < (NCOLS-1) && board[bp1_physical + 1] == PAWN))
			    ep_possible = true;
			if(!ep_possible) {
			    board[bp1_physical] = 0;
			    continue;
		    ...
		    pos[index] = bp1 + NSQUARES*(wp3 + NSQUARES*(wp2 + NSQUARES*wp1));
		    tab[bp1 + NSQUARES*(wp3 + NSQUARES*(wp2 + NSQUARES*wp1))] = index;
		    tab[bp1 + NSQUARES*(wp2 + NSQUARES*(wp3 + NSQUARES*wp1))] = index;
		    tab[bp1 + NSQUARES*(wp3 + NSQUARES*(wp1 + NSQUARES*wp2))] = index;
		    tab[bp1 + NSQUARES*(wp1 + NSQUARES*(wp3 + NSQUARES*wp2))] = index;
		    tab[bp1 + NSQUARES*(wp2 + NSQUARES*(wp1 + NSQUARES*wp3))] = index;
		    tab[bp1 + NSQUARES*(wp1 + NSQUARES*(wp2 + NSQUARES*wp3))] = index;
		    index++;
		    board[bp1_physical] = 0;
		}
		board[wp3] = 0;
	...
	fprintf(stderr,"Saw %d 3 vs 1 pawns with at least one opposing pair, expected %d\n",
	    index, N3_1_OPPOSING);
	exit(1);
    }
}
...
static void InitN1_3_OpposingTables(int *tab, int *pos)
{
	...
	for(int bp2 = bp1 + 1; bp2 < NSQUARES - NCOLS; bp2++) {
		...
		for(int wp1 = 0; wp1 < NSQUARES - NCOLS; wp1++) {
		    ...
		    if(board[wp1_physical]) continue;
		    board[wp1_physical] = PAWN;
			...
			bool ep_possible = false;
			if((Column(bp3_physical) > 0 && board[bp3_physical - 1] == PAWN) ||
			   (Column(bp3_physical) < (NCOLS-1) && board[bp3_physical + 1] == PAWN))
			    ep_possible = true;
			if(!ep_possible) {
			    board[wp1_physical] = 0;
			    continue;
		    ...
		    pos[index] = bp3 + NSQUARES*(bp2 + NSQUARES*(bp1 + NSQUARES*wp1));
		    tab[bp3 + NSQUARES*(bp2 + NSQUARES*(bp1 + NSQUARES*wp1))] = index;
		    tab[bp3 + NSQUARES*(bp1 + NSQUARES*(bp2 + NSQUARES*wp1))] = index;
		    tab[bp1 + NSQUARES*(bp3 + NSQUARES*(bp2 + NSQUARES*wp1))] = index;
		    tab[bp1 + NSQUARES*(bp2 + NSQUARES*(bp3 + NSQUARES*wp1))] = index;
		    tab[bp2 + NSQUARES*(bp3 + NSQUARES*(bp1 + NSQUARES*wp1))] = index;
		    tab[bp2 + NSQUARES*(bp1 + NSQUARES*(bp3 + NSQUARES*wp1))] = index;
		    index++;
		    board[wp1_physical] = 0;
		}
	...
	fprintf(stderr,"Saw %d 1 vs 3 pawns with at least one opposing pair, expected %d\n",
	    index, N1_3_OPPOSING);
	exit(1);
    }
}
...
int IsValidDP22(int w1, int w2, int b1, int b2)
{
    ...
    // swap black pawns so that b1 has same column as w1 and b2 the same column as w2
    ...
    if(w1_col == w2_col) {
	if(w1_col == b1_col && b1_col == b2_col && w1_row < min(b1_row, b2_row) && w2_row < max(b1_row, b2_row))
	    return ONE_COLUMN;
	return NO_DP22;
    }

    if(!(w1_col == b1_col && b1_row > w1_row && w2_col == b2_col && b2_row > w2_row))
	return NO_DP22;
...
static void InitN4OpposingTables(int *tab, int *pos)
{
	...
	for(int w2 = 0; w2 < NSQUARES; w2++) {
		...
		for(int b2_r = 0; b2_r < NROWS; b2_r++) {
		    if(tab != NULL) {
			tab[b2_r + NROWS*(b1_r + NROWS*(w2 + NSQUARES*w1))] = -1;
		    }
		}
	    }
	}
	...
	for(int w2 = w1 + 1; w2 < NSQUARES; w2++) {
		...
		for(int b2 = b1 + 1; b2 < NSQUARES; b2++) {
		    ...
		    int b2_col = Column(b2);
		    if(w1_col == b1_col) {
			pos_array_00 = Row(b2) + NROWS*(Row(b1) + NROWS*(w2 + NSQUARES*w1));
			pos_array_10 = Row(b2) + NROWS*(Row(b1) + NROWS*(w1 + NSQUARES*w2));
			pos_array_01 = Row(b1) + NROWS*(Row(b2) + NROWS*(w2 + NSQUARES*w1));
			pos_array_11 = Row(b1) + NROWS*(Row(b2) + NROWS*(w1 + NSQUARES*w2));
		    }
		    else {
			pos_array_00 = Row(b1) + NROWS*(Row(b2) + NROWS*(w2 + NSQUARES*w1));
			pos_array_10 = Row(b1) + NROWS*(Row(b2) + NROWS*(w1 + NSQUARES*w2));
			pos_array_01 = Row(b2) + NROWS*(Row(b1) + NROWS*(w2 + NSQUARES*w1));
			pos_array_11 = Row(b2) + NROWS*(Row(b1) + NROWS*(w1 + NSQUARES*w2));
		    }	
		    if(tab != NULL) {
			tab[pos_array_00] = index;
			assert(tab[pos_array_11] == -1 || tab[pos_array_11] == index);
			tab[pos_array_11] = index;
			if(w1_col == w2_col) {
			    assert(tab[pos_array_10] == -1 || tab[pos_array_10] == index);
			    tab[pos_array_10] = index;
			    assert(tab[pos_array_01] == -1 || tab[pos_array_01] == index);
    ...
    if(index != N4_OPPOSING) {
	fprintf(stderr,"Got %d pairs of opposing pawn positions, expected %d\n",
	    index, N4_OPPOSING);
	exit(1);
    }
...
static void InitN3Tables(int *tab, int *pos)
{
	...
	for(int p2 = p1; p2 < NSQUARES; p2++) {
		...
		if(p1 == p2 || p1 == p3 || p2 == p3) {
		    ...
		    if(index != g_index) {
			fprintf(stderr,"Bad triplet index: index=%d computed index=%d p1=%d p2=%d p3=%d\n",
	...
	for(int p2 = p1; p2 < NSQUARES; p2++) {
		...
		for(int p4 = p3; p4 < NSQUARES; p4++) {
		   ...
		   if(p1 == p2 || p1 == p3 || p1 == p4 ||
			...
			if(pos != NULL)
			    pos[index] = p4 + NSQUARES*(p3 + NSQUARES*(p2 + NSQUARES*p1));
			int g_index = N4_Index_Function(p4, p3, p2, p1);
			if(index != g_index) {
			    fprintf(stderr,"Bad quadruplet index: index=%d computed index=%d p1=%d p2=%d p3=%d p4=%d\n",
		   ...
		   if(tab != NULL) {
			tab[p1 + NSQUARES*(p2 + NSQUARES*(p3 + p4*NSQUARES))] = score;
			tab[p1 + NSQUARES*(p2 + NSQUARES*(p4 + p3*NSQUARES))] = score;
			tab[p1 + NSQUARES*(p3 + NSQUARES*(p2 + p4*NSQUARES))] = score;
			tab[p1 + NSQUARES*(p3 + NSQUARES*(p4 + p2*NSQUARES))] = score;
			tab[p1 + NSQUARES*(p4 + NSQUARES*(p3 + p2*NSQUARES))] = score;
			tab[p1 + NSQUARES*(p4 + NSQUARES*(p2 + p3*NSQUARES))] = score;

			tab[p2 + NSQUARES*(p1 + NSQUARES*(p3 + p4*NSQUARES))] = score;
			tab[p2 + NSQUARES*(p1 + NSQUARES*(p4 + p3*NSQUARES))] = score;
			tab[p2 + NSQUARES*(p3 + NSQUARES*(p4 + p1*NSQUARES))] = score;
			tab[p2 + NSQUARES*(p3 + NSQUARES*(p1 + p4*NSQUARES))] = score;
			tab[p2 + NSQUARES*(p4 + NSQUARES*(p1 + p3*NSQUARES))] = score;
			tab[p2 + NSQUARES*(p4 + NSQUARES*(p3 + p1*NSQUARES))] = score;

			tab[p3 + NSQUARES*(p2 + NSQUARES*(p1 + p4*NSQUARES))] = score;
			tab[p3 + NSQUARES*(p2 + NSQUARES*(p4 + p1*NSQUARES))] = score;
			tab[p3 + NSQUARES*(p1 + NSQUARES*(p4 + p2*NSQUARES))] = score;
			tab[p3 + NSQUARES*(p1 + NSQUARES*(p2 + p4*NSQUARES))] = score;
			tab[p3 + NSQUARES*(p4 + NSQUARES*(p2 + p1*NSQUARES))] = score;
			tab[p3 + NSQUARES*(p4 + NSQUARES*(p1 + p2*NSQUARES))] = score;

			tab[p4 + NSQUARES*(p2 + NSQUARES*(p3 + p1*NSQUARES))] = score;
			tab[p4 + NSQUARES*(p2 + NSQUARES*(p1 + p3*NSQUARES))] = score;
			tab[p4 + NSQUARES*(p1 + NSQUARES*(p2 + p3*NSQUARES))] = score;
			tab[p4 + NSQUARES*(p1 + NSQUARES*(p3 + p2*NSQUARES))] = score;
			tab[p4 + NSQUARES*(p3 + NSQUARES*(p1 + p2*NSQUARES))] = score;
			tab[p4 + NSQUARES*(p3 + NSQUARES*(p2 + p1*NSQUARES))] = score;
		    }
		}
	    }
	}
	...
	for(int p3 = 2; p3 <= p4; p3++) {
		...
		for(int p1 = 0; p1 <= p2; p1++) {
		   if(p1 == p2 || p1 == p3 || p1 == p4 ||
			...
			if(pos != NULL)
			    pos[index] = p4 + NSQUARES*(p3 + NSQUARES*(p2 + NSQUARES*p1));
			int g_index = p4*(p4-1)*(p4-2)*(p4-3)/24 + p3*(p3-1)*(p3-2)/6 + p2*(p2-1)/2 + p1;
			if(index != g_index) {
			    fprintf(stderr,"Bad quadruplet index: index=%d computed index=%d p1=%d p2=%d p3=%d p4=%d\n",
				index, g_index, p1, p2, p3, p4);
			    exit(1);
...
static void InitN5Tables()
{
	...
	for(int p4 = 3; p4 < p5; p4++) {
		...
		for(int p2 = 1; p2 < p3; p2++) {
			...
			if(index != g_index) {
			    fprintf(stderr,"Bad quintuplet index: index=%d computed index=%d p1=%d p2=%d p3=%d p4=%d p5=%d\n",
...
static void InitN6Tables()
{
    ...
    for(unsigned int i = 0; i <= NSQUARES; i++)
	k6_tab[i] = (i*(i-1)*(i-2)*(i-3)*(i-4)/120)*(i-5)/6;
	...
	for(int p5 = 4; p5 < p6; p5++) {
		...
		for(int p3 = 2; p3 < p4; p3++) {
		    for(int p2 = 1; p2 < p3; p2++) {
			for(int p1 = 0; p1 < p2; p1++) {
			    int g_index = N6_Index_Function(p6, p5, p4, p3, p2, p1);
			    if(index != g_index) {
				fprintf(stderr,"Bad sextuplet index: index=%d computed index=%d p1=%d p2=%d p3=%d p4=%d p5=%d p6=%d\n",
...
static void InitN7Tables()
{
    ...
    for(unsigned int i = 0; i <= NSQUARES; i++) {
	ZINDEX itmp = (i*(i-1)*(i-2)*(i-3)*(i-4)/120)*(i-5)/6;
	if(itmp % 7)
	    k7_tab[i] = itmp * ((i-6)/7);
	else
	    k7_tab[i] = (itmp / 7)* (i-6);
	...
	for(int p6 = 5; p6 < p7; p6++) {
		...
		for(int p4 = 3; p4 < p5; p4++) {
			...
			for(int p2 = 1; p2 < p3; p2++) {
			    for(int p1 = 0; p1 < p2; p1++) {
				ZINDEX g_index = N7_Index_Function(p7, p6, p5, p4, p3, p2, p1);
				if(index != g_index) {
				    fprintf(stderr,"Bad septuplet index: index=" DEC_ZINDEX_FORMAT " computed index=" DEC_ZINDEX_FORMAT " p1=%d p2=%d p3=%d p4=%d p5=%d p6=%d p7=%d\n",
				    index, g_index, p1, p2, p3, p4, p5, p6, p7);
				    exit(1);
				}
				index++;
			     }
...
static void InitPermutationTables(int count[2][KING], int bishop_parity[2])
{
	...
	if(Verbose > 1) {
	    MyPrintf("Initializing and checking permutation table for 2 opposing pawns\n");
	}
	if(k2_opposing_tab == NULL) {
	    k2_opposing_tab = (int *)MyMalloc(NSQUARES * NSQUARES * sizeof(int));
	}
	if(p2_opposing_tab == NULL) {
	    p2_opposing_tab = (int *)MyMalloc(NCOLS*(NROWS-2)*(NROWS-3)/2 * sizeof(int));
	}
	...
	if(Verbose > 1) {
	    MyPrintf("Initializing permutation tables for two vs one pawn, with one opposing pair\n");
	}
	if(k2_1_opposing_tab == NULL) {
	    k2_1_opposing_tab = (int *)MyMalloc(NSQUARES*NSQUARES*NSQUARES * sizeof(int));
	}
	...
	if(Verbose > 1) {
	    MyPrintf("Initializing permutation tables for one vs two pawns, with one opposing pair\n");
	}
	if(k1_2_opposing_tab == NULL) {
	    k1_2_opposing_tab = (int *)MyMalloc(NSQUARES*NSQUARES*NSQUARES * sizeof(int));
	}
	...
	if(Verbose > 1) {
	    MyPrintf("Initializing permutation tables for two vs two pawns, with one opposing pair\n");
	}
	if(k2_2_opposing_tab == NULL) {
	    k2_2_opposing_tab = (int *)MyMalloc(NSQUARES*NSQUARES*NSQUARES*NSQUARES * sizeof(int));
	}
	...
	if(Verbose > 1) {
	    MyPrintf("Initializing permutation tables for three vs one pawn, with one opposing pair\n");
	}
	if(k3_1_opposing_tab == NULL) {
	    k3_1_opposing_tab = (int *)MyMalloc(NSQUARES*NSQUARES*NSQUARES*NSQUARES * sizeof(int));
	}
	...
	if(Verbose > 1) {
	    MyPrintf("Initializing permutation tables for one vs three pawns, with one opposing pair\n");
	}
	if(k1_3_opposing_tab == NULL) {
	    k1_3_opposing_tab = (int *)MyMalloc(NSQUARES*NSQUARES*NSQUARES*NSQUARES * sizeof(int));
	}
	...
	if(Verbose > 1) {
	    MyPrintf("Initializing and checking permutation table for two pairs of opposing pawns\n");
	}

	if(k4_opposing_tab == NULL) {
	    k4_opposing_tab = (int *)MyMalloc(NSQUARES*NSQUARES*NSQUARES*NSQUARES * sizeof(int));
	}
	...
	for(int p = PAWN; p < KING; p++) {
		...
		if(Verbose > 1) {
		    MyPrintf("Initializing and checking permutation table for 7 pieces\n");
		}
		if(p4_tab_mb == NULL) {
		...
		if(Verbose > 1) {
		    MyPrintf("Initializing and checking permutation table for 6 pieces\n");
		}
		if(p4_tab_mb == NULL) {
		    p4_tab_mb = (int *)MyMalloc(N4*sizeof(int));
		    InitN4TablesMB(p4_tab_mb);
		...
		if(Verbose > 1) {
		    MyPrintf("Initializing and checking permutation table for 5 pieces\n");
		}
		if(p4_tab_mb == NULL) {
		    p4_tab_mb = (int *)MyMalloc(N4*sizeof(int));
		    InitN4TablesMB(p4_tab_mb);
		...
		if(Verbose > 1)
		    MyPrintf("Initializing and checking permutation tables for 4 piece\n");
#if defined(USE_PERMUTATION_FUNCTIONS)
		InitN4Tables(NULL, NULL);
#else
		if(k4_tab == NULL)
		    k4_tab = (int *)MyMalloc(NSQUARES*NSQUARES*NSQUARES*NSQUARES*sizeof(int));
		if(p4_tab == NULL)
		...
		if((p != BISHOP || bishop_parity[c] == NONE) && !m3_inited) {
		    if(Verbose > 1)
			MyPrintf("Initializing and checking permutation tables for 3 piece\n");
#if defined(USE_PERMUTATION_FUNCTIONS)
		...
		if(p == BISHOP && bishop_parity[c] == EVEN && !m3_even_inited) {
		    if(Verbose > 1)
		...
		if((p != BISHOP || bishop_parity[c] == NONE) && !m2_inited) {
		    if(Verbose > 1)
			MyPrintf("Initializing and checking permutation tables for 2 piece\n");
#if defined(USE_PERMUTATION_FUNCTIONS)
...
#if (NUM_WHITE_SQUARES) == (NUM_BLACK_SQUARES)
		if(p == BISHOP && bishop_parity[c] == EVEN && !m2_even_inited) {
		    if(Verbose > 1)
...
static ZINDEX Index11(int *pos)
{
    return pos[3] + NSQUARES*pos[2];
}

static ZINDEX IndexBP11(int *pos)
{
    return pos[2];
}
...
static ZINDEX IndexBP111(int *pos)
{
    return pos[4] + NSQUARES*pos[2];
}
...
{
    return pos[5] + NSQUARES*(pos[4] + NSQUARES*(pos[3] + NSQUARES*pos[2]));
}
...
{
    return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(pos[3] + NSQUARES*pos[2])));
}

static ZINDEX IndexBP11111(int *pos)
{
    return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(pos[2])));
}
...
static ZINDEX IndexOP11111(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(id2)));
}
...
static ZINDEX Index2_1100(int *pos)
{
...
{
    return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*N2_Index(pos[3], pos[2])));
}
...
{
    return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[4], pos[3])));
}
...
{
    return pos[6] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[5], pos[4])));
}
...
{
    return pos[4] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[6], pos[5])));
}
...
{
    return pos[6] + NSQUARES*(N2_Index(pos[5], pos[4]) + N2_Offset*N2_Index(pos[3], pos[2]));
}
...
{
    return pos[4] + NSQUARES*(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[3], pos[2]));
}
...
{
    return pos[2] + NSQUARES*(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[4], pos[3]));
}
...
{
    return pos[6] + NSQUARES*(pos[5] + NSQUARES*N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return pos[6] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[5], pos[4], pos[3]));
}
...
{
    return pos[3] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[6], pos[5], pos[4]));
}
...
{
    return N2_Index(pos[6], pos[5]) + N2_Offset*N3_Index(pos[4], pos[3], pos[2]);
}
...
{
    return N2_Index(pos[3], pos[2]) + N2_Offset*N3_Index(pos[6], pos[5], pos[4]);
}
...
{
    return (N5 - 1) - N5_Index((NSQUARES-1) - pos[2], (NSQUARES-1) - pos[3], (NSQUARES-1) - pos[4], 
				(NSQUARES-1) - pos[5], (NSQUARES-1) - pos[6]);
}
...
{
    return (N6 - 1) - N6_Index((NSQUARES-1) - pos[2], (NSQUARES-1) - pos[3], (NSQUARES-1) - pos[4], 
			       (NSQUARES-1) - pos[5], (NSQUARES-1) - pos[6], (NSQUARES-1) - pos[7]);
}
...
{
    return (N7 - 1) - N7_Index((NSQUARES-1) - pos[2], (NSQUARES-1) - pos[3], (NSQUARES-1) - pos[4], 
			       (NSQUARES-1) - pos[5], (NSQUARES-1) - pos[6], (NSQUARES-1) - pos[7],
			       (NSQUARES-1) - pos[8]);
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(pos[3] + NSQUARES*pos[2]))));
}

static ZINDEX IndexBP111111(int *pos)
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(pos[2]))));
}
...
static ZINDEX IndexOP111111(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(id2))));
}
...
{
    return pos[5] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[7], pos[6]))));
}

static ZINDEX IndexBP11112(int *pos)
{
    return pos[5] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(N2_Index(pos[7], pos[6]) + N2_Offset*pos[2]));
}
...
static ZINDEX IndexOP11112(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return pos[5] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(N2_Index(pos[7], pos[6]) + N2_Offset*id2));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[6], pos[5]))));
}

static ZINDEX IndexBP11121(int *pos)
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(N2_Index(pos[6], pos[5]) + N2_Offset*pos[2]));
}
...
static ZINDEX IndexOP11121(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return pos[7] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(N2_Index(pos[6], pos[5]) + N2_Offset*id2));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[5], pos[4]))));
}

static ZINDEX IndexBP11211(int *pos)
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(N2_Index(pos[5], pos[4]) + N2_Offset*pos[2]));
}
...
static ZINDEX IndexOP11211(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(N2_Index(pos[5], pos[4]) + N2_Offset*id2));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[4], pos[3]))));
}
...
static ZINDEX IndexOP12111(int *pos)
{
    ...
    if(op12 == ALL_ONES)
	return ALL_ONES;
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*op12));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*N2_Index(pos[3], pos[2]))));
}
...
static ZINDEX IndexOP21111(int *pos)
{
    ...
    if(op21 == ALL_ONES)
	return ALL_ONES;
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*op21));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(N2_Index(pos[5], pos[4]) + N2_Offset*N2_Index(pos[3], pos[2])));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(N2_Odd_Index(pos[3], pos[2]) + N2_ODD_PARITY_Offset*N2_Index(pos[5], pos[4])));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(N2_Even_Index(pos[3], pos[2]) + N2_EVEN_PARITY_Offset*N2_Index(pos[5], pos[4])));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[3], pos[2])));
}
...
static ZINDEX IndexOP2121(int *pos)
{
    ...
    if(op21 == ALL_ONES)
	return ALL_ONES;
    return pos[7] + NSQUARES*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*op21);
}
...
{
    return pos[5] + NSQUARES*(ZINDEX)(pos[4] + NSQUARES*(N2_Index(pos[7], pos[6]) + N2_Offset*N2_Index(pos[3], pos[2])));
}
...
static ZINDEX IndexOP2112(int *pos)
{
    ...
    if(op21 == ALL_ONES)
	return ALL_ONES;
    return pos[5] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*op21);
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[2] + NSQUARES*(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[4], pos[3])));
}
...
static ZINDEX IndexOP1221(int *pos)
{
    ...
    if(op12 == ALL_ONES)
	return ALL_ONES;
    return pos[7] + NSQUARES*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*op12);
}
...
{
    return pos[5] + NSQUARES*(ZINDEX)(pos[2] + NSQUARES*(N2_Index(pos[7], pos[6]) + N2_Offset*N2_Index(pos[4], pos[3])));
}
...
static ZINDEX IndexOP1212(int *pos)
{
    ...
    if(op12 == ALL_ONES)
	return ALL_ONES;
    return pos[5] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*op12);
}
...
{
    return pos[3] + NSQUARES*(ZINDEX)(pos[2] + NSQUARES*(N2_Index(pos[7], pos[6]) + N2_Offset*N2_Index(pos[5], pos[4])));
}

static ZINDEX IndexBP1122(int *pos)
{
    return N2_Index(pos[7], pos[6]) + N2_Offset*(ZINDEX)(N2_Index(pos[5], pos[4]) + N2_Offset*pos[2]);
}
...
static ZINDEX IndexOP1122(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return N2_Index(pos[7], pos[6]) + N2_Offset*(ZINDEX)(N2_Index(pos[5], pos[4]) + N2_Offset*id2);
}
...
{
    return N2_Index(pos[7], pos[6]) + N2_Offset*(ZINDEX)(N2_Index(pos[5], pos[4]) + N2_Offset*N2_Index(pos[3], pos[2]));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*N3_Index(pos[4], pos[3], pos[2])));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[5], pos[4], pos[3])));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[6], pos[5], pos[4])));
}

static ZINDEX IndexBP1131(int *pos)
{
    return pos[7] + NSQUARES*(ZINDEX)(N3_Index(pos[6], pos[5], pos[4]) + N3_Offset*pos[2]);
}
...
static ZINDEX IndexOP1131(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return pos[7] + NSQUARES*(ZINDEX)(N3_Index(pos[6], pos[5], pos[4]) + N3_Offset*id2);
}
...
{
    return pos[4] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[7], pos[6], pos[5])));
}

static ZINDEX IndexBP1113(int *pos)
{
    return pos[4] + NSQUARES*(ZINDEX)(N3_Index(pos[7], pos[6], pos[5]) + N3_Offset*pos[2]);
}
...
static ZINDEX IndexOP1113(int *pos)
{
    int id2 = N2_Opposing_Index(pos[3], pos[2]);
    assert(id2 != -1);
    return pos[4] + NSQUARES*(ZINDEX)(N3_Index(pos[7], pos[6], pos[5]) + N3_Offset*id2);
}
...
{
    return pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[4], pos[3]) + N2_Offset*N3_Index(pos[7], pos[6], pos[5]));
}
...
{
    return pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N3_Index(pos[5], pos[4], pos[3]));
}
...
{
    return pos[4] + NSQUARES*(ZINDEX)(N2_Index(pos[3], pos[2]) + N2_Offset*N3_Index(pos[7], pos[6], pos[5]));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(N2_Index(pos[3], pos[2]) + N2_Offset*N3_Index(pos[6], pos[5], pos[4]));
}
...
{
    return pos[5] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return N3_Index(pos[7], pos[6], pos[5]) + N3_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]);
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*N4_Index(pos[5], pos[4], pos[3], pos[2]));
}
...
{
    return pos[7] + NSQUARES*(ZINDEX)(pos[2] + NSQUARES*N4_Index(pos[6], pos[5], pos[4], pos[3]));
}
...
{
    return pos[3] + NSQUARES*(ZINDEX)(pos[2] + NSQUARES*N4_Index(pos[7], pos[6], pos[5], pos[4]));
}

static ZINDEX IndexBP114(int *pos)
{
    return N4_Index(pos[7], pos[6], pos[5], pos[4]) + N4_Offset*(ZINDEX)pos[2];
}
...
{
    return N2_Index(pos[7], pos[6]) + N2_Offset*(ZINDEX)N4_Index(pos[5], pos[4], pos[3], pos[2]);
}
...
{
    return N2_Index(pos[3], pos[2]) + N2_Offset*(ZINDEX)N4_Index(pos[7], pos[6], pos[5], pos[4]);
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[6] + NSQUARES*(ZINDEX)(pos[5] + NSQUARES*(pos[4] + NSQUARES*(pos[3] + NSQUARES*pos[2])))));
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[6] + NSQUARES*(ZINDEX)(pos[5] + NSQUARES*(pos[4] + NSQUARES*N2_Index(pos[3], pos[2])))));
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[6] + NSQUARES*(ZINDEX)(pos[5] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[4], pos[3])))));
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[6] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[5], pos[4])))));
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[6], pos[5])))));
}
...
{
   return pos[8] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[7], pos[6])))));
}
...
{
   return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N2_Index(pos[8], pos[7])))));
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[6] + NSQUARES*(ZINDEX)(N2_Index(pos[5], pos[4]) + N2_Offset*N2_Index(pos[3], pos[2]))));
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[3], pos[2]))));
}
...
{
   return pos[8] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N2_Index(pos[3], pos[2]))));
}
...
{
   return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N2_Index(pos[3], pos[2]))));
}
...
{
   return pos[8] + NSQUARES*(pos[7] + NSQUARES*(pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[4], pos[3]))));
}
...
{
   return pos[8] + NSQUARES*(pos[5] + NSQUARES*(pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N2_Index(pos[4], pos[3]))));
}
...
{
   return pos[6] + NSQUARES*(pos[5] + NSQUARES*(pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N2_Index(pos[4], pos[3]))));
}
...
{
   return pos[8] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N2_Index(pos[5], pos[4]))));
}
...
{
   return pos[6] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N2_Index(pos[5], pos[4]))));
}
...
{
   return pos[4] + NSQUARES*(pos[3] + NSQUARES*(pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N2_Index(pos[6], pos[5]))));
}
...
{
    return pos[8] + NSQUARES*(N2_Index(pos[7], pos[6]) + N2_Offset*(ZINDEX)(N2_Index(pos[5], pos[4]) + N2_Offset*N2_Index(pos[3], pos[2])));
}
...
{
    return pos[8] + NSQUARES*(N2_Odd_Index(pos[7], pos[6]) + N2_ODD_PARITY_Offset*(ZINDEX)(N2_Odd_Index(pos[3], pos[2]) + N2_ODD_PARITY_Offset*N2_Index(pos[5], pos[4])));
}
...
{
    return pos[8] + NSQUARES*(N2_Even_Index(pos[7], pos[6]) + N2_EVEN_PARITY_Offset*(ZINDEX)(N2_Odd_Index(pos[3], pos[2]) + N2_ODD_PARITY_Offset*N2_Index(pos[5], pos[4])));
}
...
{
    return pos[8] + NSQUARES*(N2_Even_Index(pos[7], pos[6]) + N2_EVEN_PARITY_Offset*(ZINDEX)(N2_Even_Index(pos[3], pos[2]) + N2_EVEN_PARITY_Offset*N2_Index(pos[5], pos[4])));
}
...
{
    return pos[6] + NSQUARES*(N2_Index(pos[8], pos[7]) + N2_Offset*(ZINDEX)(N2_Index(pos[5], pos[4]) + N2_Offset*N2_Index(pos[3], pos[2])));
}
...
{
    return pos[4] + NSQUARES*(N2_Index(pos[8], pos[7]) + N2_Offset*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[3], pos[2])));
}
...
{
    return pos[2] + NSQUARES*(N2_Index(pos[8], pos[7]) + N2_Offset*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*N2_Index(pos[4], pos[3])));
}
...
{
    return pos[8] + NSQUARES*(pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[5] + NSQUARES*N3_Index(pos[4], pos[3], pos[2]))));
}
...
{
    return pos[8] + NSQUARES*(pos[7] + NSQUARES*(ZINDEX)(pos[6] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[5], pos[4], pos[3]))));
}
...
{
    return pos[8] + NSQUARES*(pos[7] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[6], pos[5], pos[4]))));
}
...
{
    return pos[8] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[7], pos[6], pos[5]))));
}
...
{
    return pos[5] + NSQUARES*(pos[4] + NSQUARES*(ZINDEX)(pos[3] + NSQUARES*(pos[2] + NSQUARES*N3_Index(pos[8], pos[7], pos[6]))));
}
...
{
    return pos[8] + NSQUARES*(pos[7] + (NSQUARES)*(ZINDEX)(N2_Index(pos[6], pos[5]) + N2_Offset*N3_Index(pos[4], pos[3], pos[2])));
}
...
{
    return pos[8] + NSQUARES*(pos[5] + (NSQUARES)*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N3_Index(pos[4], pos[3], pos[2])));
}
...
{
    return pos[8] + NSQUARES*(pos[5] + (NSQUARES)*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N3_Odd_Index(pos[4], pos[3], pos[2])));
}
...
{
    return pos[8] + NSQUARES*(pos[5] + (NSQUARES)*(ZINDEX)(N2_Odd_Index(pos[7], pos[6]) + N2_ODD_PARITY_Offset*N3_Odd_Index(pos[4], pos[3], pos[2])));
}
...
{
    return pos[8] + NSQUARES*(pos[5] + (NSQUARES)*(ZINDEX)(N2_Even_Index(pos[7], pos[6]) + N2_EVEN_PARITY_Offset*N3_Odd_Index(pos[4], pos[3], pos[2])));
}
...
{
    return pos[6] + NSQUARES*(pos[5] + (NSQUARES)*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N3_Index(pos[4], pos[3], pos[2])));
}
...
{
    return pos[8] + NSQUARES*(pos[7] + (NSQUARES)*(ZINDEX)(N2_Index(pos[3], pos[2]) + N2_Offset*N3_Index(pos[6], pos[5], pos[4])));
}
...
{
    return pos[8] + NSQUARES*(pos[4] + (NSQUARES)*(ZINDEX)(N2_Index(pos[3], pos[2]) + N2_Offset*N3_Index(pos[7], pos[6], pos[5])));
}
...
{
    return pos[5] + NSQUARES*(pos[4] + (NSQUARES)*(ZINDEX)(N2_Index(pos[3], pos[2]) + N2_Offset*N3_Index(pos[8], pos[7], pos[6])));
}
...
{
    return pos[8] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N3_Index(pos[5], pos[4], pos[3])));
}
...
{
    return pos[6] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N3_Index(pos[5], pos[4], pos[3])));
}
...
{
    return pos[6] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Even_Index(pos[8], pos[7]) + N2_EVEN_PARITY_Offset*N3_Index(pos[5], pos[4], pos[3])));
}
...
{
    return pos[6] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Odd_Index(pos[8], pos[7]) + N2_ODD_PARITY_Offset*N3_Index(pos[5], pos[4], pos[3])));
}
...
{
    return pos[8] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Index(pos[4], pos[3]) + N2_Offset*N3_Index(pos[7], pos[6], pos[5])));
}
...
{
    return pos[5] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Index(pos[4], pos[3]) + N2_Offset*N3_Index(pos[8], pos[7], pos[6])));
}
...
{
    return pos[3] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N3_Index(pos[6], pos[5], pos[4])));
}
...
{
    return pos[3] + NSQUARES*(pos[2] + (NSQUARES)*(ZINDEX)(N2_Index(pos[5], pos[4]) + N2_Offset*N3_Index(pos[8], pos[7], pos[6])));
}
...
{
    return pos[8] + NSQUARES*(N3_Index(pos[7], pos[6], pos[5]) + N3_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return pos[8] + NSQUARES*(N3_Even_Index(pos[7], pos[6], pos[5]) + N3_EVEN_PARITY_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return pos[8] + NSQUARES*(N3_Odd_Index(pos[7], pos[6], pos[5]) + N3_ODD_PARITY_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return pos[5] + NSQUARES*(N3_Index(pos[8], pos[7], pos[6]) + N3_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return pos[2] + NSQUARES*(N3_Index(pos[8], pos[7], pos[6]) + N3_Offset*(ZINDEX)N3_Index(pos[5], pos[4], pos[3]));
}
...
{
    return N2_Index(pos[8], pos[7]) + (N2_Offset)*(N2_Index(pos[6], pos[5]) + N2_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return N2_Even_Index(pos[6], pos[5]) + (N2_EVEN_PARITY_Offset)*(N2_Index(pos[8], pos[7]) + N2_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return N2_Odd_Index(pos[6], pos[5]) + (N2_ODD_PARITY_Offset)*(N2_Index(pos[8], pos[7]) + N2_Offset*(ZINDEX)N3_Index(pos[4], pos[3], pos[2]));
}
...
{
    return N2_Index(pos[8], pos[7]) + (N2_Offset)*(N2_Index(pos[3], pos[2]) + N2_Offset*(ZINDEX)N3_Index(pos[6], pos[5], pos[4]));
}
...
{
    return N2_Index(pos[5], pos[4]) + (N2_Offset)*(N2_Index(pos[3], pos[2]) + (N2_Offset)*(ZINDEX)N3_Index(pos[8], pos[7], pos[6]));
}
...
{
    return N2_Odd_Index(pos[3], pos[2]) + (N2_ODD_PARITY_Offset)*(N2_Index(pos[5], pos[4]) + (N2_Offset)*(ZINDEX)N3_Index(pos[8], pos[7], pos[6]));
}
...
{
    return N2_Even_Index(pos[3], pos[2]) + (N2_EVEN_PARITY_Offset)*(N2_Index(pos[5], pos[4]) + (N2_Offset)*(ZINDEX)N3_Index(pos[8], pos[7], pos[6]));
}
...
{
    return pos[8] + (NSQUARES)*(pos[7] + (NSQUARES)*(pos[6] + (NSQUARES)*(ZINDEX)N4_Index(pos[5], pos[4], pos[3], pos[2])));
}
...
{
    return pos[8] + (NSQUARES)*(pos[7] + (NSQUARES)*(pos[2] + (NSQUARES)*(ZINDEX)N4_Index(pos[6], pos[5], pos[4], pos[3])));
}
...
{
    return pos[8] + (NSQUARES)*(pos[3] + (NSQUARES)*(pos[2] + (NSQUARES)*(ZINDEX)N4_Index(pos[7], pos[6], pos[5], pos[4])));
}
...
{
    return pos[4] + (NSQUARES)*(pos[3] + (NSQUARES)*(pos[2] + (NSQUARES)*(ZINDEX)N4_Index(pos[8], pos[7], pos[6], pos[5])));
}
...
{
    return pos[8] + NSQUARES*(ZINDEX)(N2_Index(pos[7], pos[6]) + N2_Offset*N4_Index(pos[5], pos[4], pos[3], pos[2]));
}
...
{
    return pos[8] + NSQUARES*(ZINDEX)(N2_Even_Index(pos[7], pos[6]) + N2_EVEN_PARITY_Offset*N4_Index(pos[5], pos[4], pos[3], pos[2]));
}
...
{
    return pos[8] + NSQUARES*(ZINDEX)(N2_Odd_Index(pos[7], pos[6]) + N2_ODD_PARITY_Offset*N4_Index(pos[5], pos[4], pos[3], pos[2]));
}
...
{
    return pos[6] + NSQUARES*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N4_Index(pos[5], pos[4], pos[3], pos[2]));
}
...
{
    return pos[8] + NSQUARES*(ZINDEX)(N2_Index(pos[3], pos[2]) + N2_Offset*N4_Index(pos[7], pos[6], pos[5], pos[4]));
}
...
{
    return pos[4] + NSQUARES*(ZINDEX)(N2_Index(pos[3], pos[2]) + N2_Offset*N4_Index(pos[8], pos[7], pos[6], pos[5]));
}
...
{
    return pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[8], pos[7]) + N2_Offset*N4_Index(pos[6], pos[5], pos[4], pos[3]));
}
...
{
    return pos[2] + NSQUARES*(ZINDEX)(N2_Index(pos[4], pos[3]) + N2_Offset*N4_Index(pos[8], pos[7], pos[6], pos[5]));
}
...
{
    return N3_Index(pos[8], pos[7], pos[6]) + N3_Offset*(ZINDEX)N4_Index(pos[5], pos[4], pos[3], pos[2]);
}
...
{
    return N3_Index(pos[4], pos[3], pos[2]) + N3_Offset*(ZINDEX)N4_Index(pos[8], pos[7], pos[6], pos[5]);
}
...
typedef struct {
    ...
    int parity;
    int pawn_file_type;
    IndexType *eptr_bp_11, *eptr_op_11, *eptr_op_21, *eptr_op_12, *eptr_dp_22, *eptr_op_22, 
		*eptr_op_31, *eptr_op_13, *eptr_op_41, *eptr_op_14, *eptr_op_32, *eptr_op_23,
		*eptr_op_33, *eptr_op_42, *eptr_op_24;
    ZINDEX index_bp_11, index_op_11, index_op_21, index_op_12, index_dp_22, index_op_22, 
		index_op_31, index_op_13, index_op_41, index_op_14, index_op_32, index_op_23,
		index_op_33, index_op_42, index_op_24;
    int num_pieces;
    int kk_index;
} MB_INFO;
...
static unsigned int Promotions = (1 << KNIGHT) | (1 << BISHOP) | (1 << ROOK) | (1 << QUEEN);
...
static int
GetEndingName(int type_count[2][KING], char *ending)
{
    int len = 0, piece;
...
static int
GetEnPassant(int *pos, int *types, int npieces)
{
    if(UseEnPassant) {
	int i;
   	for(i = 2; i < npieces; i++) {
...
static void
fill_single_side(int loc, int nleft, bool count_only)
{
    int i;
	...
	if(!count_only) {
	    memcpy(SingleSideCount[num_side_endings].types, single_piece_side, sizeof(single_piece_side));
...
InitEndingStats()
{
    ...
    SingleSideCount = (ENDING_INDEX *)MyMalloc(num_side_endings*sizeof(ENDING_INDEX));
    ...
    qsort(SingleSideCount, num_side_endings, sizeof(ENDING_INDEX), side_compare);
    
    num_total_endings = num_side_endings*num_side_endings;

    EndingStats = (unsigned int *)MyMalloc(num_total_endings * sizeof(unsigned int));
...
static int ending_stat_index(int piece_types[2][KING])
{
    ...
    eptr = (ENDING_INDEX *)bsearch(&single_side, SingleSideCount, num_side_endings, sizeof(ENDING_INDEX), side_compare);
...
static void
GetEndingFromIndex(int piece_types[2][KING], int index)
{
    int id_b, id_w, i;
	...
	if(SingleSideCount[i].index == id_w) {
	    memcpy(piece_types[0], SingleSideCount[i].types, KING*sizeof(int));
	    break;
	}
    }
	...
	if(SingleSideCount[i].index == id_b) {
	    memcpy(piece_types[1], SingleSideCount[i].types, KING*sizeof(int));
	    break;
	}
    }
}
...
{
    if(PGN_Game == NULL) {
	if((PGN_Game = (PGN_OUTPUT *)malloc(MAX_LINES * sizeof(PGN_OUTPUT)))==NULL) {
	   fprintf(stderr,"Could not allocate %d lines for PGN output\n", MAX_LINES);
	   exit(1);
	}
    }
    if(ScoreListPGN == NULL) {
	if((ScoreListPGN = (SCORE *)malloc(MAX_GAME_MOVES * sizeof(SCORE)))==NULL) {
	   fprintf(stderr,"Could not allocate %d scores\n", MAX_GAME_MOVES);
	   exit(1);
	}
    }
    if(ScoreListDeltaPGN == NULL) {
	if((ScoreListDeltaPGN = (SCORE *)malloc(MAX_GAME_MOVES * sizeof(SCORE)))==NULL) {
	   fprintf(stderr,"Could not allocate %d scores\n", MAX_GAME_MOVES);
	   exit(1);
	}
    }
...
static int
OutputPGNCommentString(char *string)
{
    int len = strlen(string);
    bool append_blank = false;
	...
	for(;;) {
	    ...
	    int line_start = 0;
	    // make sure new line does not start with %, and for chessbase also not with [Tag
	    // where Tag is one of the seven standard tags
	    if(string[offset] == '%' ||
	    ...
	    // the enhanced PGN standard uses [% inside a comment as a special comment symbol.
	    // Make sure that symbol does not get split between lines
	    if(string[offset + LineWidth - 1 - line_start] == '[' && string[offset + LineWidth - line_start] == '%') {
		len_to_write--;
	    }
...
static int
OutputPGNStringCashed(char *string)
{
    int len = strlen(string);
    bool append_blank = false;
...
static int
OutputPGNCommentStringCashed(char *string)
{
    int len = strlen(string);
    bool append_blank = false;
...
static int
OutputPGNCommentCashed(char *comment)
{
    static bool first_call = true;
    static char *string;
    ...
    if(first_call) {
	if((string = (char *)malloc(MAX_PGN_LINE*sizeof(char)))==NULL) {
	    fprintf(stderr,"OutputPGNCommentCashed: could not allocate string\n");
	    exit(1);
	}
	first_call = false;
    }
             ...
             int half_move, int full_move)
{
    ...
    for(i = 0; i < NSQUARES; i++) {
   	if(board[i] > 0) {
		...
		Board->piece_locations[WHITE][board[i]][Board->piece_type_count[WHITE][board[i]]++] = i;
		nwhite++;
		...
		Board->piece_locations[BLACK][-board[i]][Board->piece_type_count[BLACK][-board[i]]++] = i;
		nblack++;
...
static void
DisplayRawBoard(int *board, char *label)
{
    int row, col, sq, pi;
	...
	for(col = 0; col < NCOLS; col++) {
	    ...
	    MyPrintf(" ");
	    if(pi) {
		MyPrintf("%c", (pi > 0) ? toupper(piece_char(pi)) :
		                          piece_char(-pi));
	    }
	    else
		MyPrintf(".");
...
static void
DisplayBoard(BOARD *Board, char *label)
{
    int row, col, sq, pi;
...
{
    return !NonBlankLine(line);
}

static char *
NextInputLine(FILE *fp)
{
...
static LinePair
GatherString(char *line, unsigned char *linep, char *string)
{
...
static LinePair
GatherSimpleString(char *line, unsigned char *linep, char *string)
{
    LinePair resulting_line;
    unsigned len = 0;
    char *str_end;
 ...
 */
static LinePair
GatherComment(char *line, unsigned char *linep, FILE *fp, char *comment)
{
    LinePair resulting_line;
    char ch;
...
static int
IdentifyTag(const char *tag_string)
{
    int tag_index;
 ...
 */
static LinePair
GatherTag(char *line, unsigned char *linep, FILE *fp, int *tag_index)
{
    LinePair resulting_line;
    char ch;
    unsigned len = 0;
...
static int GetPiece(char p)
{
	...
	case 'm':
	case 'M': return MAHARAJA; break;
	case 'q':
	case 'Q': return QUEEN; break;
	case 'c':
	case 'C': return CARDINAL; break;
	case 'r':
	case 'R': return ROOK; break;
	case 'a':
	case 'A': return ARCHBISHOP; break;
	case 'b':
...
static int
ParseEndingName(char *ending, int count[2][KING])
{
...
static bool
IsCastlingCharacter(char c)
{
    return (c == 'O') || (c == '0') || (c == 'o');
}

static bool IsCheck(char c)
{
    return (c == '+') || (c == '#');
}
...
bool
DecodeMove(char *move_string, Move *move_details, Move *move_list, int nmoves)
{
    ...
    else if(*move == '-' || *move == 'Z') {  /* allow possibility of null move -- */
	char prior_move = *move;
	move++;
	if((prior_move == '-' && *move == '-') || (prior_move == 'Z' && *move == '0')) {
	    mclass = NULL_MOVE;
	    move++;
	}
	else {
	...
	// Because of chess 960, don't want to match castling because a king move
	// may match it unless the "mclass" is a castling move
 ...
 */
static LinePair
GatherPossibleNumeric(char *line, unsigned char *linep, char initial_digit,
                      ParseType *yylval)
{
    LinePair resulting_line;
	...
	(void) sscanf((const char *)yylval->string_info,"%d",&yylval->move_number);
 ...
 */
static PGNToken
GetNextToken(FILE *fp, ParseType *yylval)
{
	...
	yylval->string_info[0] = '\0';
		...
		resulting_line = GatherSimpleString(line,linep,yylval->string_info);
		/* Pick up where we are now. */
		line = resulting_line.line;
		linep = resulting_line.linep;
		token = resulting_line.token;
		...
		resulting_line = GatherComment(line,linep,fp,yylval->string_info);
		/* Pick up where we are now. */
		line = resulting_line.line;
		linep = resulting_line.linep;
		token = resulting_line.token;
		 ...
		 * Remember that 1 can start 1-0 and 1/2.
		 */
		resulting_line = GatherPossibleNumeric(
		                    line,linep,next_char, yylval);
		/* Pick up where we are now. */
		line = resulting_line.line;
		linep = resulting_line.linep;
		token = resulting_line.token;
		...
		if(!skipping_game){
		    if(Verbose > 1 || CheckSyntaxOnly)
			MyPrintf("Internal error: Missing case for %d on char %x.\n",
                            token,next_char);
		}
		token = NO_TOKEN;
            }
...
PGNToken
SkipToNextGame(PGNToken token, FILE *fp, ParseType *yylval)
{
    if(SkipToken(token)) {
	skipping_game = true;
	do {
...
PGNToken
ParseGamePrefix(PGNToken symbol, FILE *fp, ParseType *yylval)
{
    while(symbol != TAG && symbol != EOF_TOKEN) {
	symbol = GetNextToken(fp, yylval);
	if(symbol == COMMENT && !CheckSyntaxOnly && !PositionDatabase && !PrintBadlyPlayedPositions) {
	    if(Verbose > 3)
...
static void InitCaches()
{
	...
	FileCacheHighDTZ[i][0].starting_index = FileCacheHighDTZ[i][1].starting_index = NULL;
...
int MyUncompress(BYTE *dest, ULONG *dest_size,
		const BYTE *source, ULONG source_size, int method)
{
	...
	*dest_size = source_size;
	...
	if(ZSTD_DecompressionContext == NULL)
	    destCapacity = ZSTD_decompress(dest, destCapacity, source, source_size);
	else
	    destCapacity = ZSTD_decompressDCtx(ZSTD_DecompressionContext, 
				dest, destCapacity, source, source_size);
	...
	*dest_size = destCapacity;
	...
	if(err_code != Z_OK) {
		...
		if(err_code == Z_MEM_ERROR)
		    ...
		    MyPrintf("ZLIB uncompress: not enough room in output buffer\n");
...
static int
GetEndingType(int count[2][KING], int *piece_types, int bishop_parity[2], int pawn_file_type)
{
	...
	if(pawn_file_type == OP_41_PAWNS || pawn_file_type == OP_14_PAWNS ||
	   ...
	   pawn_file_type == OP_33_PAWNS || pawn_file_type == OP_42_PAWNS || pawn_file_type == OP_24_PAWNS)
	    pawn_file_type_effective = FREE_PAWNS;

	for(int i = 0; i < NumIndexTypes; i++) {
	    if(IndexTable[i].etype == etype && IndexTable[i].op_type == pawn_file_type_effective) {
		eindex = i;
		break;
	    }
	}
	...
	if(bishop_parity[WHITE] != NONE) {
		...
		fprintf(stderr,"Can only fix white bishop parity if white has two or three bishops\n");
		exit(1);
	    }
	}
	...
	if(bishop_parity[BLACK] != NONE) {
		...
		fprintf(stderr,"Can only fix black bishop parity if black has two or three bishops\n");
		exit(1);
	    }
	}
	...
	for(int i = 0; i < NumIndexTypes; i++) {
	    if(IndexTable[i].etype == etype && IndexTable[i].sub_type == sub_type && IndexTable[i].op_type == FREE_PAWNS) {
		eindex = i;
		break;
	    }
	}
...
static int
GetEndingTypeYK(int count[2][KING], int *piece_types)
{
    ...
    for(int i = 0; i < NumIndexTypes; i++) {
	if(IndexTable[i].etype == etype && IndexTable[i].sub_type == 0 && IndexTable[i].op_type == FREE_PAWNS) {
	   eindex = i;
	   break;
	}
    }
...
bool IsResultChangingMove(int score)
{
    if(score == PLUS_EQUAL || score == EQUAL_MINUS || score == PLUS_MINUS ||
       score == PLUS_EQUALMINUS || score == EQUALMINUS_PLUS || score == PLUSEQUAL_MINUS || 
       score == EQUAL_PLUS || score == MINUS_PLUS || score == MINUS_EQUAL || score == MINUS_PLUSEQUAL) {
	return true;
    }
    return false;
}
 ...
 * result as a move as well. For example, if Black resigns after a white move, this
 * can be considered a move by black that leads to a WON position by White.
...
{
    if(score == UNKNOWN || score == UNRESOLVED || score == DRAW || score == ILLEGAL)
	return score;
    else if(score == NOT_WON)
	return NOT_LOST;
    else if(score == NOT_LOST)
...
{
    if(prev_score == UNKNOWN || prev_score == UNRESOLVED || curr_score == UNKNOWN || curr_score == UNRESOLVED)
	return 0;
	...
	if(implied_score == DRAW || implied_score == NOT_LOST || implied_score == NOT_WON)
	    return 0;
	if(implied_score == LOST || implied_score <= 0)
	    return EQUAL_MINUS;
	return EQUAL_PLUS;
	...
	if(implied_score == DRAW || implied_score <= 0 || implied_score == NOT_LOST || 
	   implied_score == LOST || implied_score == NOT_WON)
	    return 0;
	...
	if(implied_score == LOST || implied_score == NOT_WON || implied_score <= 0)
	    return 0;
	if(implied_score == DRAW)
	    return MINUS_EQUAL;
...
static bool IsWinningScore(int score)
{
    ...
    if(!(score == UNKNOWN || score == DRAW || score == LOST || score == NOT_WON || score == NOT_LOST || score == UNRESOLVED) &&
	score > 0)
	return true;
...
static bool IsLosingScore(int score)
{
    ...
    if(!(score == UNKNOWN || score == DRAW || score == WON || score == NOT_WON || score == NOT_LOST || score == UNRESOLVED) &&
 	score <= 0)
	return true;
...
static int
ScoreCompare(int score1, int score2)
{
    if(score1 == score2)
	return 0;
    ...
    if(score1 == WON) {
	if(score2 < 0 || score2 == DRAW || score2 == LOST || score2 == NOT_WON ||
	   score2 == STALE_MATE || score2 == CHECK_MATE || score2 == UNRESOLVED)
	    return -1;
	return 1;
    }
    ...
    if(score2 == NOT_WON || score2 == DRAW || score2 == NOT_LOST || score2 == LOST) {
	if(score1 >= 0)
	    return -1;
	return 1;
    }
...
static int
MoveCompare(const void *v1, const void *v2)
{
    Move *m1 = (Move *)v1;
    Move *m2 = (Move *)v2;
    ...
    if((m1->flag & (CAPTURE | PROMOTION)) && !(m2->flag & (CAPTURE | PROMOTION))) {
	if(IsWinningScore(m1->score))
	    return -1;
    }
    if((m2->flag & (CAPTURE | PROMOTION)) && !(m1->flag & (CAPTURE | PROMOTION))) {
...
MakeMove(BOARD *Board, Move *mv)
{
	...
	if((mv->flag & PROMOTION) && mv->piece_promoted != PAWN) {
	    ...
	    Board->strength_w += (PieceStrengths[mv->piece_promoted] -
	                          PieceStrengths[PAWN]);
	}
	else {
	...
	if(mv->piece_moved == KING) {
		...
		if(Verbose > 6) {
		    ...
		    MyPrintf("  %d = 0, %d = %d\n",
		       ...
		       SquareMake(0, ROOK_GCASTLE_DEST_COL),
		       ROOK);
		}		    		
		...
		if(Verbose > 6) {
		    ...
		    MyPrintf("  %d = 0, %d = %d\n",
		       ...
		       SquareMake(0, ROOK_CCASTLE_DEST_COL),
		       ROOK);
		}		    		
		...
		if((col_to >= 1 && board[SquareMake(3, col_to - 1)] == -PAWN) ||
		   (col_to <= NCOLS-2 && board[SquareMake(3, col_to + 1)] == -PAWN))
	...
	if((mv->flag & PROMOTION) && mv->piece_promoted != -PAWN) {
	    ...
	    promo_pos[type_count[-mv->piece_promoted]++] = mv->to;
	    Board->strength_b += (PieceStrengths[-mv->piece_promoted]-
	                          PieceStrengths[PAWN]);
	}
	else {
	    for(i = 0; i < type_count[-mv->piece_moved]; i++) {
	...
	if(mv->piece_moved == -KING) {
		...
		for(i = 0; i < Board->piece_type_count[BLACK][ROOK]; i++) {
			...
			br_pos[i] = SquareMake(NROWS-1, ROOK_GCASTLE_DEST_COL);
			break;
		    }
		}
	    }    
		...
		for(i = 0; i < Board->piece_type_count[BLACK][ROOK]; i++) {
			...
			br_pos[i] = SquareMake(NROWS-1, ROOK_CCASTLE_DEST_COL);
			break;
		    }
		}
	    }    
		...
		if((col_to >= 1 && board[SquareMake(NROWS-4, col_to - 1)] == PAWN) ||
		   (col_to <= NCOLS-2 && board[SquareMake(NROWS-4, col_to + 1)] == PAWN))
    ...
    if(mv->piece_moved == PAWN || mv->piece_moved == -PAWN || (mv->flag & CAPTURE))
	Board->half_move = 0;
    else
    	Board->half_move++;
}
...
static void
MakeMoveUCI(BOARD *Board, char *mv)
{
    int *board = Board->board;
	...
	if((flag & PROMOTION) && piece_promoted != PAWN) {
	    ...
	    Board->strength_w += (PieceStrengths[piece_promoted] -
	                          PieceStrengths[PAWN]);
	}
	else {
	...
	if((flag & PROMOTION) && piece_promoted != -PAWN) {
	    ...
	    promo_pos[type_count[-piece_promoted]++] = to;
	    Board->strength_b += (PieceStrengths[-piece_promoted]-
	                          PieceStrengths[PAWN]);
	}
	else {
	    for(int i = 0; i < type_count[-piece_moved]; i++) {
...
static void
UnMakeMove(BOARD *Board, Move *mv)
{
    int i;
    int *board = Board->board;
	...
	if(mv->piece_moved == -KING) {
		...
		board[mv->from] = -KING;
		for(i = 0; i < Board->piece_type_count[BLACK][ROOK]; i++) {
		    if(br_pos[i] == SquareMake(NROWS-1, ROOK_GCASTLE_DEST_COL)) {
		...
		board[mv->from] = -KING;
		for(i = 0; i < Board->piece_type_count[BLACK][ROOK]; i++) {
		    if(br_pos[i] == SquareMake(NROWS-1, ROOK_CCASTLE_DEST_COL)) {
...
static int
GetMoveStringBleicher(Move *mv, char *str)
{
    int from = mv->from;
    int to = mv->to;
...
static int
GetMoveString(Move *mv, char *str, bool simple)
{
	...
	sprintf(str,"%c%c%d%c%c%d",
	p,
	'a'+Column(mv->from),1+Row(mv->from),
	(mv->flag & CAPTURE) ? 'x' : '-',
...
static int
GetShortMoveString(Move *mv, Move *mv_list, int nmoves, char *str)
{
    int i, len = 0;
    char p;
...
static int
CastleRights(int *board, int proposed_castle)
{
    int castle = 0;
	...
	if(wk_castle) {
		...
		if(wrcol != -1) {
			...
			if(nrooks > 1) {
			    if(Verbose > 1) {
				printf("Warning: more than one rook availble for white k-side castling\n");
			    }
			}
			castle |= WK_CASTLE;
		    }
		...
		if(wrcol != -1) {
			...
			if(nrooks > 1) {
			    if(Verbose > 1) {
				printf("Warning: more than one rook availble for white q-side castling\n");
			    }
			}
			castle |= WQ_CASTLE;
		    }
	...
	if(bk_castle) {
		...
		if(brcol != -1) {
			...
			if(nrooks > 1) {
			    if(Verbose > 1) {
				printf("Warning: more than one rook availble for black k-side castling\n");
			    }
			}
			castle |= BK_CASTLE;
		    }
		...
		if(brcol != -1) {
			...
			if(nrooks > 1) {
			    if(Verbose > 1) {
				printf("Warning: more than one rook availble for black q-side castling\n");
			    }
			}
			castle |= BQ_CASTLE;
		    }
...
static int ReadPositionData(char *line, POSITION_DATA *pos_data)
{
    ...
    int nwords = sscanf(line,"%s %d " DEC_INDEX_FORMAT " %c %d %d %c %s %s %s",
		     ending, &kk_index, &offset, &side, &game_num, &move_no, &result, score_string, zz_string, cz_string);
    if(line[0] == '#' || nwords == 0)
	return 0;
...
static int ScanPosition(char *pos_string, int *board, int *ep_square, int *castle, char *title)
{
...
scan_piece:
	...
	for(;;) {
		...
		if(board[SquareMake(row-1,col)] != -PAWN) {
		    if(Verbose > 1) {
			MyPrintf("No black pawn available to be e.p. captured on %c%d\n",
			     'a'+col,1+row);
		    }
		    *ep_square = 0;
		    break;
		...
		if(col > 0 && board[SquareMake(row-1,col-1)] == PAWN ||
		   col < (NCOLS-1) && board[SquareMake(row-1,col+1)] == PAWN)
		    ok = true;
		if(!ok) {
		    if(Verbose > 1) {
			MyPrintf("No white pawn available to e.p. capture on %c%d\n",
			         'a'+col,1+row);
		    }
		    *ep_square = 0;
		    break;
		...
		if(board[SquareMake(row+1,col)] != PAWN) {
		    if(Verbose > 1) {
			MyPrintf("No white pawn available to be e.p. captured on %c%d\n",
			     'a'+col,1+row);
		    }
		    break;
		}
		...
		if(!ok) {
		    if(Verbose > 1) {
			MyPrintf("No black pawn available to e.p. capture on %c%d\n",
			         'a'+col,1+row);
		    }
		    break;
		}
...
static bool
IsAttacked(int *board, int attacker, int ptype, int victim)
{
    int atype;
...
static bool
IsInCheck(BOARD *Board, int side)
{
    if(side == WHITE) {
...
static int
ScanFEN(char *fen_string, int *board, int *ep_square, int *castle, int *half_move, int *full_move, char *title)
{
	...
	*pptr = '\0';
    ...
    n = sscanf(fen_string,"%s %s %s %s %s %s",pos_str, side_str, castle_str, ep_str,
               half_move_str, full_move_str);
...
castle_done:	*castle = avail_castle;
    }
    ...
    *ep_square = 0;
	...
	for(;;) {
	    ...
	    if(col < 0 || col >= NCOLS || row < 0 | row >= NROWS) {
		if(Verbose > 1) {
		    MyPrintf("ScanFEN: Warning: e.p. square %c%d out of range\n",'a'+col,1+row);
		}
		break;
	    }
		...
		if(row != NROWS - 3) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: e.p. square %c%d on wrong row for wtm\n",
			        'a'+col,1+row);
		    }
		    break;
		}
		if(board[SquareMake(row, col)] != 0) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: e.p. square %c%d occupied\n",'a'+col,1+row);
		    }
		    break;
		}
		if(board[SquareMake(row+1, col)] != 0) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: e.p. starting square %c%d occupied\n",'a'+col,1+row+1);
		    }
		    break;
		}
		if(board[SquareMake(row-1,col)] != -PAWN) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: no black pawn available to be e.p. captured on %c%d\n",
			     'a'+col,1+row);
		    }
		    break;
		}
		...
		if(!ok) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: no white pawn available to e.p. capture on %c%d\n",
			         'a'+col,1+row);
		    }
		    break;
		}
		*ep_square = SquareMake(row, col);
		...
		if(row != 2) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: e.p. square %c%d on wrong row for btm\n",
			        'a'+col,1+row);
		    }
		    break;
		}
		...
		if(board[SquareMake(row+1,col)] != PAWN) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: no white pawn available to be e.p. captured on %c%d\n",
			     'a'+col,1+row);
		    }
		    break;
		}
		...
		if(!ok) {
		    if(Verbose > 1) {
			MyPrintf("ScanFEN: Warning: no black pawn available to e.p. capture on %c%d\n",
			         'a'+col,1+row);
		    }
		    break;
		}
...
static int
GetBoardFromMB(BOARD *Board, char *ending, int side, int kk_index, ZINDEX offset)
{
    ...
    eindex = GetEndingType(piece_type_count, piece_types, bishop_parity, pawn_file_type);

    if(eindex == -1)
	return 0;

    bool pawns_present = piece_type_count[WHITE][PAWN] ||
                         piece_type_count[BLACK][PAWN];
	...
	if(Verbose > 0)
	    MyPrintf("Could not obtain position from ending %d, offset " DEC_ZINDEX_FORMAT "\n",
	         eindex, offset);
	return 0;
    }
    ...
    n_pieces = SetBoard(Board, board, side, ep_square, castle, half_move,
                        full_move);
...
static int ReadPositionList(char *pos, BOARD *Board, POSITION_DATA *pos_data)
{
    ...
    if(Verbose > 1)
       MyPrintf("game: %d  result: %d score: %d  zz_type: %d\n", 
           pos_data->game_num, Board->result, Board->score, Board->zz_type);
...
static int ConvertMB_to_YK(char *line)
{
    ...
    int eindex = GetEndingType(piece_type_count, piece_types, bishop_parity, pawn_file_type);
    ...
    MyPrintf("%s %d " DEC_ZINDEX_FORMAT " %s", pos_data.ending, kk_index, offset_yk, rest);
...
{ 
    ...
    MyPrintf("%s %d " DEC_ZINDEX_FORMAT " %s", pos_data.ending, kk_index, offset, rest);
...
{
    int legal, board[NSQUARES], ep_square = 0, castle = 0, half_move = 0, full_move = 1;
	...
	legal = ScanFEN(pos, board, &ep_square, &castle, &half_move, &full_move, title);
    }
    else {
	if(Verbose > 3) {
...
static void
PrintPieceTypeCount(int types[2][KING], char *comment)
{
    int piece;
...
static void
PrintPosition(int *pos, int *types, int n_pieces)
{
    int i;
...
static void
BoardToFEN(BOARD *Board, char *FEN_String)
{
    char *pch;
    int sq, sqLast, i;
    int *board = Board->board;
...
static void
PrintEPD(BOARD *Board, char *comment)
{
    char fen_string[256];
 ...
 * a "canonical" configuration when pawns are present.  The symmetry operation index 
 * and transformed king positions are stored in sym, wk_in, and bk_in respectively.
 *
...
/*
 * KK_Canonical_NoPawns computes the symmetry operation to transform wk_in and bk_in to
 * a "canonical" configuration without pawns.  The symmetry operation index and 
 * transformed king positions are stored in sym, wk_in, and bk_in respectively.
 *
 * The routine returns true if the position is legal, false otherwise
 */
...
#if defined(SQUARE)
	if(wk_trans_row < (NROWS+1)/2 && wk_trans_col < (NCOLS+1)/2 && wk_trans_row <= wk_trans_col) {
#  if defined(ODD_SQUARE)
	    if(wk_trans_row == wk_trans_col) {
		if(wk_trans_row == NROWS/2) {
		    if(bk_trans_row <= NROWS/2 && bk_trans_col <= NCOLS/2 && bk_trans_row <= bk_trans_col)
			sym_found = true;
		}
...
static void InitTransforms()
{
	...
	for(int sym = 0; sym < NSYMMETRIES; sym++) {
		...
		fprintf(stderr,"for symmetry transformation %s, sq = %d, inverse of trans = %d\n",
	...
	fprintf(stderr,"KK_Transform_Table already initialzed in InitTransform\n");
	exit(1);
    }
	...
	fprintf(stderr,"KK_Transform_Table_NoPawns already initialzed in InitTransform\n");
	exit(1);
    }
    if(KK_Index_Table_NoPawns != NULL) {
	fprintf(stderr,"KK_Index_Table_NoPawns already initialzed in InitTransform\n");
	exit(1);
    }

    KK_Transform_Table_NoPawns = (int *)MyMalloc(NSQUARES*NSQUARES*sizeof(int));
...
static void InitMoves()
{
	...
	n = 0;
	for(row2 = row + 1, col2 = col+1; row2 < NROWS && col2 < NCOLS; row2++, col2++)
	    BishopMoves[sq][0][n++] = SquareMake(row2, col2);
	BishopMoves[sq][0][n] = -1;
	n = 0;
	for(row2 = row + 1, col2 = col-1; row2 < NROWS && col2 >= 0; row2++, col2--)
	    BishopMoves[sq][1][n++] = SquareMake(row2, col2);
	BishopMoves[sq][1][n] = -1;
	n = 0;
	for(row2 = row - 1, col2 = col+1; row2 >= 0 && col2 < NCOLS; row2--, col2++)
	    BishopMoves[sq][2][n++] = SquareMake(row2, col2);
	BishopMoves[sq][2][n] = -1;
	n = 0;
	for(row2 = row - 1, col2 = col-1; row2 >= 0 && col2 >= 0; row2--, col2--)
...
static int GenPseudoLegalMoves(BOARD *Board, Move *move_list, bool skip_pawn_promotion)
{
	...
	for(ptype = PAWN; ptype < KING; ptype++) {    
	    ...
	    memcpy(locations, Board->piece_locations[WHITE][ptype], sizeof(locations));
	    int *pos = locations;
		...
		if(ptype == PAWN) {
			...
			if(1) {
				...
				int promo_type;			    
				for(promo_type = PAWN + 1; promo_type < KING; promo_type++) {
			...
			if(Board->board[dest] < 0 || (Board->ep_square > 0 && dest == Board->ep_square)) {
			    move_list->from = pos[i];
			    move_list->to = dest;
			    move_list->piece_moved = PAWN;
			    move_list->flag = CAPTURE;
			    if(Board->ep_square > 0 && dest == Board->ep_square) {
	...
	if(Board->castle & WK_CASTLE) {
		...
		if(Verbose > 1) {
		    char label[64];
		    sprintf(label,"wkpos=%d, does not allow k-side castling", Board->wkpos);
	    ...
	    int cstart = min(kstart, min(grook_orig_col, ROOK_GCASTLE_DEST_COL));
	    int cend = max(kend, max(grook_orig_col, ROOK_GCASTLE_DEST_COL));
	    for(i = cstart; i <= cend; i++) {
		if(i == king_orig_col || i == grook_orig_col) continue;	    
	...
	if(Board->castle & WQ_CASTLE) {
		...
		if(Verbose > 1) {
		    char label[64];
		    sprintf(label,"wkpos=%d, does not allow q-side castling", Board->wkpos);
		    DisplayBoard(Board, label);
		}
		castle_possible = false;
	    }
	    ...
	    int cstart = min(kstart, min(crook_orig_col, ROOK_CCASTLE_DEST_COL));
	    int cend = max(kend, max(crook_orig_col, ROOK_CCASTLE_DEST_COL));
	    for(i = cstart; i <= cend; i++) {
		if(i == king_orig_col || i == crook_orig_col) continue;	    
		if(Board->board[SquareMake(0, i)] != 0) {
		    castle_possible = false;
		    break;
		}
	...
	for(ptype = PAWN; ptype < KING; ptype++) {    
	    int locations[MAX_IDENT_PIECES];
	    memcpy(locations, Board->piece_locations[BLACK][ptype], sizeof(locations));
	    int *pos = locations;
	...
	if(Board->castle & BK_CASTLE) {
		...
		if(Verbose > 1) {
		    char label[64];
		    sprintf(label,"bkpos=%d, does not allow k-side castling", Board->bkpos);
		    DisplayBoard(Board, label);
		}
		castle_possible = false;
	    }
		...
		if(castle_possible) {
		    ...
		    move_list->to = SquareMake(NROWS-1, KING_GCASTLE_DEST_COL);
		    move_list++;
		    nmoves++;
		}
	    }
	...
	if(Board->castle & BQ_CASTLE) {
		...
		if(Verbose > 1) {
		    char label[64];
		    sprintf(label,"bkpos=%d, does not allow q-side castling", Board->bkpos);
		    DisplayBoard(Board, label);
		}
		castle_possible = false;
	    }
		...
		if(castle_possible) {
		    ...
		    move_list->to = SquareMake(NROWS-1, KING_CCASTLE_DEST_COL);
		    move_list++;
		    nmoves++;
		}
	    }
	...
	for(ptype = PAWN; ptype < KING; ptype++) {    
		...
		if(ptype == PAWN) {
			...
			if(!is_in_check) {
				...
				int promo_type;			    
				for(promo_type = KING-1; promo_type > PAWN; promo_type--) {
...
static int 
GenLegalFromPseudoMoves(BOARD *curr_pos, Move *move_list, Move *pseudo_move_list, 
                        int npseudo_moves)
{
    int nmoves = 0;
...
static int
GetMBPosition(BOARD *Board, int *mb_position, int *parity, int *pawn_file_type)
{
	...
	for(int i = 0; i < Board->piece_type_count[color][PAWN]; i++) {
		...
		if(color == WHITE && SquareMake(Row(pos[i])-1,Column(pos[i])) == Board->ep_square)
		    mb_position[loc] = SquareMake(0, Column(pos[i]));
		if(color == BLACK && SquareMake(Row(pos[i])+1, Column(pos[i])) == Board->ep_square)
    ...
    *pawn_file_type = FREE_PAWNS;
    ...
    if(Board->piece_type_count[WHITE][PAWN] == 1 && Board->piece_type_count[BLACK][PAWN] == 1) {
    ...
    else if(Board->piece_type_count[WHITE][PAWN] == 2 && Board->piece_type_count[BLACK][PAWN] == 1) {
	int op21 = N2_1_Opposing_Index(mb_position[4], mb_position[3], mb_position[2]);
	if(op21 != -1)
	   *pawn_file_type = OP_21_PAWNS;
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 1 && Board->piece_type_count[BLACK][PAWN] == 2) {
	int op12 = N1_2_Opposing_Index(mb_position[4], mb_position[3], mb_position[2]);
	if(op12 != -1)
	   *pawn_file_type = OP_12_PAWNS;
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 2 && Board->piece_type_count[BLACK][PAWN] == 2) {
	ZINDEX dp22 = IndexDP22(mb_position);
	if(Verbose > 3) {
	    MyPrintf("GetMBPosition: wp1 = %c%d wp2 = %c%d bp1 = %c%d bp2 = %c%d, sp22 = " DEC_ZINDEX_FORMAT "\n",
	...
	if(dp22 != ALL_ONES)
	   *pawn_file_type = DP_22_PAWNS;
	    ...
	    int op22 = N2_2_Opposing_Index(mb_position[5], mb_position[4], mb_position[3], mb_position[2]);
	    if(op22 != -1)
		*pawn_file_type = OP_22_PAWNS;
	}
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 3 && Board->piece_type_count[BLACK][PAWN] == 1) {
	int op31 = N3_1_Opposing_Index(mb_position[5], mb_position[4], mb_position[3], mb_position[2]);
	if(op31 != -1)
	   *pawn_file_type = OP_31_PAWNS;
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 1 && Board->piece_type_count[BLACK][PAWN] == 3) {
	int op13 = N1_3_Opposing_Index(mb_position[5], mb_position[4], mb_position[3], mb_position[2]);
	if(op13 != -1)
	   *pawn_file_type = OP_13_PAWNS;
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 4 && Board->piece_type_count[BLACK][PAWN] == 1) {
	if((Column(mb_position[6]) == Column(mb_position[2]) && mb_position[2] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[3]) && mb_position[3] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[4]) && mb_position[4] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[5]) && mb_position[5] < mb_position[6])) {
	   *pawn_file_type = OP_41_PAWNS;
	}
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 1 && Board->piece_type_count[BLACK][PAWN] == 4) {
	if((Column(mb_position[2]) == Column(mb_position[3]) && mb_position[2] < mb_position[3]) ||
	   (Column(mb_position[2]) == Column(mb_position[4]) && mb_position[2] < mb_position[4]) ||
	   (Column(mb_position[2]) == Column(mb_position[5]) && mb_position[2] < mb_position[5]) ||
	   (Column(mb_position[2]) == Column(mb_position[6]) && mb_position[2] < mb_position[6])) {
	    *pawn_file_type = OP_14_PAWNS;
	}
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 3 && Board->piece_type_count[BLACK][PAWN] == 2) {
	if((Column(mb_position[5]) == Column(mb_position[2]) && mb_position[2] < mb_position[5]) ||
	   (Column(mb_position[5]) == Column(mb_position[3]) && mb_position[3] < mb_position[5]) ||
	   (Column(mb_position[5]) == Column(mb_position[4]) && mb_position[4] < mb_position[5]) ||
	   (Column(mb_position[6]) == Column(mb_position[2]) && mb_position[2] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[3]) && mb_position[3] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[4]) && mb_position[4] < mb_position[6])) {
	   *pawn_file_type = OP_32_PAWNS;
	}
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 2 && Board->piece_type_count[BLACK][PAWN] == 3) {
	if((Column(mb_position[2]) == Column(mb_position[4]) && mb_position[2] < mb_position[4]) ||
	   (Column(mb_position[2]) == Column(mb_position[5]) && mb_position[2] < mb_position[5]) ||
	   (Column(mb_position[2]) == Column(mb_position[6]) && mb_position[2] < mb_position[6]) ||
	   (Column(mb_position[3]) == Column(mb_position[4]) && mb_position[3] < mb_position[4]) ||
	   (Column(mb_position[3]) == Column(mb_position[5]) && mb_position[3] < mb_position[5]) ||
	   (Column(mb_position[3]) == Column(mb_position[6]) && mb_position[3] < mb_position[6])) {
	    *pawn_file_type = OP_23_PAWNS;
	}
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 3 && Board->piece_type_count[BLACK][PAWN] == 3) {
	if((Column(mb_position[5]) == Column(mb_position[2]) && mb_position[2] < mb_position[5]) ||
	   (Column(mb_position[5]) == Column(mb_position[3]) && mb_position[3] < mb_position[5]) ||
	   (Column(mb_position[5]) == Column(mb_position[4]) && mb_position[4] < mb_position[5]) ||
	   (Column(mb_position[6]) == Column(mb_position[2]) && mb_position[2] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[3]) && mb_position[3] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[4]) && mb_position[4] < mb_position[6]) ||
	   (Column(mb_position[7]) == Column(mb_position[2]) && mb_position[2] < mb_position[7]) ||
	   (Column(mb_position[7]) == Column(mb_position[3]) && mb_position[3] < mb_position[7]) ||
	   (Column(mb_position[7]) == Column(mb_position[4]) && mb_position[4] < mb_position[7])) {
	   *pawn_file_type = OP_33_PAWNS;
	}
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 4 && Board->piece_type_count[BLACK][PAWN] == 2) {
	if((Column(mb_position[6]) == Column(mb_position[2]) && mb_position[2] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[3]) && mb_position[3] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[4]) && mb_position[4] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[5]) && mb_position[5] < mb_position[6]) ||
	   ...
	   (Column(mb_position[7]) == Column(mb_position[3]) && mb_position[3] < mb_position[7]) ||
	   (Column(mb_position[7]) == Column(mb_position[4]) && mb_position[4] < mb_position[7]) ||
	   (Column(mb_position[7]) == Column(mb_position[5]) && mb_position[5] < mb_position[7])) {
	   *pawn_file_type = OP_42_PAWNS;
	}
    }
    else if(Board->piece_type_count[WHITE][PAWN] == 2 && Board->piece_type_count[BLACK][PAWN] == 4) {
	if((Column(mb_position[4]) == Column(mb_position[2]) && mb_position[2] < mb_position[4]) ||
	   (Column(mb_position[4]) == Column(mb_position[3]) && mb_position[3] < mb_position[4]) ||
	   (Column(mb_position[5]) == Column(mb_position[2]) && mb_position[2] < mb_position[5]) ||
	   (Column(mb_position[5]) == Column(mb_position[3]) && mb_position[3] < mb_position[5]) ||
	   (Column(mb_position[6]) == Column(mb_position[2]) && mb_position[2] < mb_position[6]) ||
	   (Column(mb_position[6]) == Column(mb_position[3]) && mb_position[3] < mb_position[6]) ||
	   (Column(mb_position[7]) == Column(mb_position[2]) && mb_position[2] < mb_position[7]) ||
	...
	bishops_on_black_squares[color] = Board->piece_type_count[color][BISHOP] -
			bishops_on_white_squares[color];
    }
    ...
    *parity = 1000*bishops_on_white_squares[WHITE] +
	...
	MyPrintf("GetMBPosition: Bad number of pieces: %d, expected %d",
	    loc, Board->num_pieces);
...
static int
GetYKPosition(BOARD *Board, int *yk_position)
{
    int loc = 0;
	...
	for(int type = KING-1; type >= PAWN; type--) {
		...
		if(type == PAWN && UseEnPassant && Board->ep_square > 0) {
		    if(color == WHITE &&
		       SquareMake(Row(pos[i])-1, Column(pos[i])) == Board->ep_square)
		       yk_position[loc] = SquareMake(0, Column(pos[i]));

		    if(color == BLACK &&
		       SquareMake(Row(pos[i])+1, Column(pos[i])) == Board->ep_square)
		       yk_position[loc] = SquareMake(NROWS-1, Column(pos[i]));
		}
		loc++;
	    }
	}
    ...
    if(loc != Board->num_pieces) {
	MyPrintf("Bad number of pieces in GetYKPosition: %d, expected %d",
	    loc, Board->num_pieces);
	DisplayBoard(Board, NULL);
	exit(1);
    }
...
static ZINDEX
GetMBIndex(int *mb_pos, int npieces, bool pawns_present, IndexType *eptr, int *kindex, ZINDEX *offset)
{
    if(eptr == NULL) {
	*kindex = -1;
	*offset = ALL_ONES;
    ...
    if(Verbose > 3) {
	MyPrintf("GetMBIndex: For wk=%c%d bk=%c%d, pawns present=%d, transform needed (%d) = %s\n",
	  'a'+Column(wk),1+Row(wk),
	  'a'+Column(bk),1+Row(bk),
	  pawns_present,
	  sym,
	  SymmetryName[sym]);
    }
...
static int
GetYKIndex(int *yk_pos, int npieces, bool pawns_present, IndexType *eptr, int *kindex, ZINDEX *offset)
{
    if(eptr == NULL) {
	*kindex = -1;
	*offset = ALL_ONES;
    ...
    if(Verbose > 3) {
	MyPrintf("GetYKIndex: For wk=%c%d bk=%c%d, pawns present=%d, transform needed (%d) = %s\n",
	  'a'+Column(wk),1+Row(wk),
	  'a'+Column(bk),1+Row(bk),
	  pawns_present,
	  sym,
...
static int
ParseBoard(int *board, int ep_square, int count[2][KING], int *pos, int *types)
{
	...
	if(piece == PAWN && sq == ep_pawn) {
		...
		pos[PieceLocation[color][piece]++] = SquareMake(NROWS-1, Column(sq));
	}
	else
...
static void
GetYKBaseFileName(int count[2][KING], int side, char *fname)
{
    fname[0] = 'k';
    int len = 1;
    for(int piece = KING-1; piece >= PAWN; piece--) {	
...
static file
OpenMBFile(char *ending, int kk_index, int bishop_parity[2], int pawn_file_type, int side, bool high_dtz)
{
    char path[1024];

    if(Verbose > 3) {
	MyPrintf("OpenMBFile: ending=%s, kk_index=%d, w_parity = %s, b_parity = %s, pawn_file_type = %d, side = %s, NumPaths = %d, high_dtz = %d\n",
	   ending, kk_index,
	   ((bishop_parity[WHITE] == NONE) ? "none" : ((bishop_parity[WHITE] == EVEN) ? "even" : "odd")),
	   ((bishop_parity[BLACK] == NONE) ? "none" : ((bishop_parity[BLACK] == EVEN) ? "even" : "odd")),
	   pawn_file_type,
	   ColorName(side), NumPaths, high_dtz);
    }
	...
	if(bishop_parity[WHITE] == NONE && bishop_parity[BLACK] == NONE) {
	...
	else if(bishop_parity[WHITE] != NONE && bishop_parity[BLACK] == NONE) {
	    ...
	    assert(bishop_parity[WHITE] != NONE && bishop_parity[BLACK] != NONE);
	    snprintf(dirname, sizeof(dirname) - 1, "%s_%s_%s", ending,
		((bishop_parity[WHITE] == EVEN) ? "wbe" : "wbo"),
		((bishop_parity[BLACK] == EVEN) ? "bbe" : "bbo"));
	}

	snprintf(path, sizeof(path) - 1, "%s%c%s_out%c%s_%c_%d.%s", 
	    TbPaths[i], DELIMITER[0], dirname, DELIMITER[0],
	    ending, (side == WHITE ? 'w' : 'b'), kk_index,
	    (high_dtz ? "hi" : "mb"));
	if(Verbose > 3) {
...
OpenYKFile(char *base_name)
{
    ...
    for(int i = 0; i < NumPaths; i++) {
	snprintf(path, sizeof(path)-1, "%s%c%s", TbPaths[i], DELIMITER[0], base_name);
	if(Verbose > 3) {
...
static int
GetMBInfo(BOARD *Board, MB_INFO *mb_info)
{
    ...
    memcpy(mb_info->piece_type_count, Board->piece_type_count, sizeof(Board->piece_type_count));
    ...
    GetMBPosition(Board, mb_info->mb_position, &mb_info->parity, &mb_info->pawn_file_type);

    memset(mb_info->mb_piece_types, 0, sizeof(mb_info->mb_piece_types));
    
    int eindex = GetEndingType(Board->piece_type_count, mb_info->mb_piece_types, bishop_parity, FREE_PAWNS);
    ...
    if(eindex >= 0) {
	memcpy(mb_info->parity_index[0].bishop_parity, bishop_parity, sizeof(bishop_parity));
	...
	if(mb_info->pawn_file_type == OP_11_PAWNS || mb_info->pawn_file_type == BP_11_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_11_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 1 opposing pawn pair = %d\n", eindex);
		}
		mb_info->eptr_op_11 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_11,
			&kk_index_blocked, &mb_info->index_op_11);
	    }
	    else {
		if(Verbose > 4) {
	...
	if(mb_info->pawn_file_type == BP_11_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, BP_11_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 1 pair of blocking pawns = %d\n", eindex);
		}
		mb_info->eptr_bp_11 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_bp_11,
			&kk_index_blocked, &mb_info->index_bp_11);
	    }
	    else {
		if(Verbose > 4) {
	...
	if(mb_info->pawn_file_type == OP_21_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_21_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 2 vs 1 pawn, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_21 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_21,
			&kk_index_blocked, &mb_info->index_op_21);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 2 vs 1 pawn, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_12_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_12_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 1 vs 2 pawns, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_12 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_12,
			&kk_index_blocked, &mb_info->index_op_12);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 1 vs 2 pawns, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_22_PAWNS || mb_info->pawn_file_type == DP_22_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_22_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with pawn pairs, with 1 opposing pairs = %d\n", eindex);
		}
		mb_info->eptr_op_22 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_22,
			&kk_index_blocked, &mb_info->index_op_22);
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: kk_index_blocked = %d, index = " DEC_ZINDEX_FORMAT "\n",
			kk_index_blocked, mb_info->index_op_22);
		}
	    }
	    else {
	...
	if(mb_info->pawn_file_type == DP_22_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, DP_22_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 2 opposing pawn pairs = %d\n", eindex);
		}
		mb_info->eptr_dp_22 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_dp_22,
	...
	if(mb_info->pawn_file_type == OP_31_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_31_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 3 vs 1 pawn, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_31 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_31,
			&kk_index_blocked, &mb_info->index_op_31);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 3 vs 1 pawn, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_13_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_13_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 1 vs 3 pawns, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_13 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_13,
			&kk_index_blocked, &mb_info->index_op_13);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 1 vs 3 pawns, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_41_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_41_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 4 vs 1 pawn, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_41 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_41,
			&kk_index_blocked, &mb_info->index_op_41);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 4 vs 1 pawn, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_14_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_14_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 1 vs 4 pawns, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_14 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_14,
			&kk_index_blocked, &mb_info->index_op_14);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 1 vs 4 pawns, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_32_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_32_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 3 vs 2 pawn, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_32 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_32,
			&kk_index_blocked, &mb_info->index_op_32);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 3 vs 2 pawn, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_23_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_23_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 2 vs 3 pawns, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_23 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_23,
			&kk_index_blocked, &mb_info->index_op_23);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 2 vs 3 pawns, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_33_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_33_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 3 vs 3 pawn, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_33 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_33,
			&kk_index_blocked, &mb_info->index_op_33);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 3 vs 3 pawn, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_42_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_42_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 4 vs 2 pawn, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_42 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_42,
			&kk_index_blocked, &mb_info->index_op_42);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 4 vs 2 pawn, 1 opposing pawn pair\n");
		}
	...
	if(mb_info->pawn_file_type == OP_24_PAWNS) {
	    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, OP_24_PAWNS);
	    if(eindex >= 0) {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: eindex with 2 vs 4 pawn, one opposing pair = %d\n", eindex);
		}
		mb_info->eptr_op_24 = &IndexTable[eindex];
		GetMBIndex(mb_info->mb_position, mb_info->num_pieces, true, mb_info->eptr_op_24,
			&kk_index_blocked, &mb_info->index_op_24);
	    }
	    else {
		if(Verbose > 4) {
		    MyPrintf("GetMBInfo: no eindex for 2 vs 4 pawn, 1 opposing pawn pair\n");
		}
	...
	// for odd-sized boards, can't do any parities for triples, and only odd parities for doubles

#if (NROWS % 2) && (NCOLS % 2)
	if(bishop_parity[WHITE] != NONE) {
	    if(mb_info->piece_type_count[WHITE][BISHOP] >= 3 || bishop_parity[WHITE] == EVEN)
		bishop_parity[WHITE] = NONE;
	}

	if(bishop_parity[BLACK] != NONE) {
	    if(mb_info->piece_type_count[BLACK][BISHOP] >= 3 || bishop_parity[BLACK] == EVEN)
	...
	GetMBIndex(mb_info->mb_position, mb_info->num_pieces, pawns_present, mb_info->parity_index[0].eptr, 
		&mb_info->kk_index, &mb_info->parity_index[0].index);
	if(kk_index_blocked != -1) {
	    if(kk_index_blocked != mb_info->kk_index) {
		fprintf(stderr,"GetMBInfo: kk_index_blocked = %d, kk_index = %d\n",
    ...
    eindex = GetEndingType(Board->piece_type_count, NULL, bishop_parity, FREE_PAWNS);

    if(Verbose > 4) {
	MyPrintf("GetMBInfo: eindex %d with white bishop parity %s, black bishop parity %s\n", eindex,
	    ((bishop_parity[WHITE] == NONE) ? "none" : ((bishop_parity[WHITE] == EVEN) ? "even" : "odd")),
	    ((bishop_parity[BLACK] == NONE) ? "none" : ((bishop_parity[BLACK] == EVEN) ? "even" : "odd")));
    }

    if(eindex >= 0) {
	memcpy(mb_info->parity_index[mb_info->num_parities].bishop_parity, bishop_parity, sizeof(bishop_parity));
	mb_info->parity_index[mb_info->num_parities].eptr = &IndexTable[eindex];
	mb_info->num_parities++;
	InitPermutationTables(mb_info->piece_type_count, bishop_parity);
    }

    // if both white and black have parity constraints, can add cases where only one side is constrained
	...
	eindex = GetEndingType(Board->piece_type_count, NULL, sub_bishop_parity, FREE_PAWNS);
	...
	if(eindex >= 0) {
	    memcpy(mb_info->parity_index[mb_info->num_parities].bishop_parity, sub_bishop_parity, sizeof(sub_bishop_parity));
	    mb_info->parity_index[mb_info->num_parities].eptr = &IndexTable[eindex];
	    mb_info->num_parities++;
    ...
    if(mb_info->num_parities == 0) {
	if(Verbose > 4) {
	    MyPrintf("GetMBInfo: Could not map to index type, for piece type counts:\n");
	...
	int kk_index;
	GetMBIndex(mb_info->mb_position, mb_info->num_pieces, pawns_present, mb_info->parity_index[i].eptr,
		&kk_index, &mb_info->parity_index[i].index);
...
static int
LookupScoreInDatabase(BOARD *Board)
{
    if(Verbose > 4) {
	...
	if(mb_info.pawn_file_type == OP_21_PAWNS)
	    MyPrintf("   index 2 vs 1 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_21);
	if(mb_info.pawn_file_type == OP_12_PAWNS)
	    MyPrintf("   index 1 vs 2 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_12);
	if(mb_info.pawn_file_type == DP_22_PAWNS)
	...
	if(mb_info.pawn_file_type == OP_31_PAWNS)
	    MyPrintf("   index 3 vs 1 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_31);
	if(mb_info.pawn_file_type == OP_13_PAWNS)
	    MyPrintf("   index 1 vs 3 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_13);
	for(int i = 0; i < mb_info.num_parities; i++) {
	    MyPrintf("  index %d: " DEC_ZINDEX_FORMAT "\n", i, mb_info.parity_index[i].index);
	}
    }
    ...
    POSITION_DB *pptr = (POSITION_DB *)bsearch(&pos, PositionDB, NumDBPositions, sizeof(POSITION_DB), db_pos_compar);

    if(pptr != NULL) {
	if(Verbose > 3) {
	    MyPrintf("LookupScoreInDatabase: found position, score=%d, promos=%d\n", pptr->score, pptr->promos);
	    MyFlush();
	}
    ...
    if(result < 0) {
	if(Verbose > 3) {
	    MyPrintf("LookupScoreInDatabase: returning negative result after flip\n");
	    MyFlush();
	}
	return result;
    }
    ...
    pptr = (POSITION_DB *)bsearch(&pos, PositionDB, NumDBPositions, sizeof(POSITION_DB), db_pos_compar);

    if(pptr != NULL) {
	if(Verbose > 3) {
	    MyPrintf("LookupScoreInDatabase: found position after flipping, score=%d, promos=%d\n", pptr->score, pptr->promos);
	    MyFlush();
	}
	Board->score = pptr->score;
	Board->zz_type = pptr->zz_type;
...
static int
GetYKInfo(BOARD *Board, YK_INFO *yk_info)
{
    ...
    memcpy(yk_info->piece_type_count, Board->piece_type_count, sizeof(Board->piece_type_count));
    ...
    int eindex = GetEndingTypeYK(Board->piece_type_count, yk_info->yk_piece_types);
    ...
    GetYKIndex(yk_info->yk_position, yk_info->num_pieces, pawns_present, yk_info->eptr, 
		&yk_info->kk_index, &yk_info->index);
...
static int
GetMBCacheResult(MB_INFO *mb_info, int side)
{
	...
	if(fcache->kk_index != mb_info->kk_index)
	    continue;
	if(memcmp(fcache->piece_type_count, mb_info->piece_type_count, sizeof(fcache->piece_type_count)))
	    continue;
	...
	for(int i = 0; i < mb_info->num_parities; i++) {
		...
		(fcache->bishop_parity[WHITE] == mb_info->parity_index[i].bishop_parity[WHITE])) &&
	       (fcache->bishop_parity[BLACK] == NONE || 
		(fcache->bishop_parity[BLACK] == mb_info->parity_index[i].bishop_parity[BLACK]))) {
		found_parity = true;
	...
	if(!found_parity) {
	    ...
	    if(mb_info->pawn_file_type == BP_11_PAWNS) {
		if(mb_info->index_bp_11 != ALL_ONES && fcache->pawn_file_type == BP_11_PAWNS) {
		    index = mb_info->index_bp_11;
		    continue;
		}
	    }
	    if(mb_info->pawn_file_type == OP_21_PAWNS) {
		if(mb_info->index_op_21 != ALL_ONES && fcache->pawn_file_type == OP_21_PAWNS) {
		    index = mb_info->index_op_21;
		    continue;
		}
	    }
	    if(mb_info->pawn_file_type == OP_12_PAWNS) {
		if(mb_info->index_op_12 != ALL_ONES && fcache->pawn_file_type == OP_12_PAWNS) {
		    index = mb_info->index_op_12;
		    continue;
		}
	    }
	    if(mb_info->pawn_file_type == OP_22_PAWNS || mb_info->pawn_file_type == DP_22_PAWNS) {
		if(mb_info->index_op_22 != ALL_ONES && fcache->pawn_file_type == OP_22_PAWNS) {
		    index = mb_info->index_op_22;
		    continue;
		}
	    }
	    if(mb_info->pawn_file_type == DP_22_PAWNS) {
		if(mb_info->index_dp_22 != ALL_ONES && fcache->pawn_file_type == DP_22_PAWNS) {
		    index = mb_info->index_dp_22;
		    continue;
		}
	    }
	    if(mb_info->pawn_file_type == OP_31_PAWNS) {
		if(mb_info->index_op_31 != ALL_ONES && fcache->pawn_file_type == OP_31_PAWNS) {
		    index = mb_info->index_op_31;
		    continue;
		}
	    }
	    if(mb_info->pawn_file_type == OP_13_PAWNS) {
		if(mb_info->index_op_13 != ALL_ONES && fcache->pawn_file_type == OP_13_PAWNS) {
	...
	for(n = 0; n < num_cached_files_high_dtz[side]; n++) {
		...
		continue;
	    if(memcmp(fcache_high_dtz->piece_type_count, mb_info->piece_type_count, 
		...
		if((fcache_high_dtz->bishop_parity[WHITE] == NONE || 
		    (fcache_high_dtz->bishop_parity[WHITE] == mb_info->parity_index[i].bishop_parity[WHITE])) &&
		   (fcache_high_dtz->bishop_parity[BLACK] == NONE || 
		    (fcache_high_dtz->bishop_parity[BLACK] == mb_info->parity_index[i].bishop_parity[BLACK]))) {
		    found_parity = true;
		    index = mb_info->parity_index[i].index;
		    break;
		}
		...
		if(mb_info->pawn_file_type == BP_11_PAWNS && fcache_high_dtz->pawn_file_type == BP_11_PAWNS) {
		...
		if(mb_info->pawn_file_type == OP_21_PAWNS && fcache_high_dtz->pawn_file_type == OP_21_PAWNS) {
		...
		if(mb_info->pawn_file_type == OP_12_PAWNS && fcache_high_dtz->pawn_file_type == OP_12_PAWNS) {
		...
		if(mb_info->pawn_file_type == DP_22_PAWNS && fcache_high_dtz->pawn_file_type == DP_22_PAWNS) {
		...
		if(mb_info->pawn_file_type == OP_31_PAWNS && fcache_high_dtz->pawn_file_type == OP_31_PAWNS) {
		...
		if(mb_info->pawn_file_type == OP_13_PAWNS && fcache_high_dtz->pawn_file_type == OP_13_PAWNS) {
	...
	// if index is outside range of all indices with depth > 254, depth must be 254
	if(index < fcache_high_dtz->starting_index[0] || 
	   index > fcache_high_dtz->starting_index[fcache_high_dtz->header.num_blocks]) {
	    CacheHits++;
	    return 254;
	}
	...
	ULONG n_per_block =
	    (fcache_high_dtz->header.n_elements + fcache_high_dtz->header.num_blocks - 1) / 
		    fcache_high_dtz->header.num_blocks;

	if(fcache_high_dtz->block_index == fcache_high_dtz->header.num_blocks - 1) {
	    ULONG rem = fcache_high_dtz->header.n_elements % n_per_block;
	    if(rem != 0)
		n_per_block = rem;
	}

	// check whether index is in range of cached block.  If not, return unknown

	HIGH_DTZ *hptr = (HIGH_DTZ *)fcache_high_dtz->block;
	if(index < hptr[0].index || index > hptr[n_per_block - 1].index)
	    return UNKNOWN;

	// now perform binary search on block that may contain score corresponding to index
	HIGH_DTZ key;
	key.index = index;

	HIGH_DTZ *kptr = (HIGH_DTZ *)bsearch(&key, fcache_high_dtz->block, n_per_block,
...
static int
GetYKCacheResult(YK_INFO *yk_info, int side)
{
    // check whether file for ending is already opened
	...
	if(memcmp(fcache->piece_type_count, yk_info->piece_type_count, sizeof(fcache->piece_type_count)))
	    continue;
	...
	HDATA *tptr = (HDATA *)bsearch(&tdata, fcache->block_high, fcache->num_high_dtc, sizeof(HDATA), CompareHigh);
	if(tptr != NULL) {
	    CacheHits++;
	    return tptr->dtc;
	}
...
static int
GetYKResult(BOARD *Board, INDEX_DATA *ind)
{
    YK_INFO yk_info;
	...
	if(Verbose > 3) {
	    MyPrintf("GetYKResult: offset " DEC_ZINDEX_FORMAT " larger than 64^5\n", yk_info.index);
	}
	...
	if(num_blocks > fcache->max_num_blocks) {
		...
		MyFree(fcache->offsets, (fcache->max_num_blocks + 1) * sizeof(INDEX));
	    }
	    fcache->max_num_blocks = num_blocks;
	    fcache->offsets = (INDEX *)MyMalloc((fcache->max_num_blocks + 1)*sizeof(INDEX));
	...
	if(f_read(fcache->offsets, (num_blocks+1)*sizeof(INDEX), file_yk, YK_HEADER_SIZE) != (num_blocks+1)*sizeof(INDEX)) {
	    if(Verbose > 2) {
		MyPrintf("Could not read %d offsets from %s\n",num_blocks+1, base_name);
	    }
	    f_close(file_yk);
	...
	if(max_depth > 254) {
	    ...
	    INDEX num_high_dtc = (((INDEX)(num_high_dtc_high)) << 32) + num_high_dtc_low;
	    fcache->num_high_dtc = num_high_dtc;
	}

	memcpy(fcache->piece_type_count, yk_info.piece_type_count, sizeof(yk_info.piece_type_count));
    ...
    if(Verbose > 4) {
	MyPrintf("Searching YK index %lu block index %lu, header block size %lu\n",
	...
	if(Verbose > 4) {
	    MyPrintf("Reading compressed block size %lu at offset " DEC_INDEX_FORMAT " block index %lu\n",
	...
	if(length > CompressionBufferSize) {
	    ...
	    if(CompressionBuffer == NULL) {
		fprintf(stderr,"Could not allocate CompressionBuffer size %lu\n", CompressionBufferSize);
		exit(1);
	    }
	}
	...
	MyUncompress(fcache->block, &tmp_zone_size, CompressionBuffer, length, fcache->compression_method);
	...
	if(fcache->block_high == NULL) {
		...
		fcache->block_high = (HDATA *)MyMalloc(fcache->num_high_dtc * sizeof(HDATA));
		if(fcache->block_high == NULL) {
		    if(Verbose > 2) {
			MyPrintf("Could not allocate " DEC_INDEX_FORMAT " high DTC scores\n", fcache->num_high_dtc);
		    }
		    return HIGH_DTZ_MISSING;
		}
		INDEX nread = f_read(fcache->block_high, fcache->num_high_dtc * sizeof(HDATA), fcache->fp_high, 0);
		if(nread != fcache->num_high_dtc * sizeof(HDATA)) {
		    if(Verbose > 2) {
			MyPrintf("Read " DEC_INDEX_FORMAT " bytes from high DTC file, expected " DEC_INDEX_FORMAT "\n",
			    nread, fcache->num_high_dtc * sizeof(HDATA));
		    }
		    return HIGH_DTZ_MISSING;
		}
		qsort(fcache->block_high, fcache->num_high_dtc, sizeof(HDATA), CompareHigh);
	    }
	    else {
		return HIGH_DTZ_MISSING;
	    }
...
static int
GetMBResult(BOARD *Board, INDEX_DATA *ind)
{
    if(Verbose > 3) {
	...
	if(mb_info.pawn_file_type == OP_13_PAWNS)
	    MyPrintf("   index 1 vs 3 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_13);
	if(mb_info.pawn_file_type == OP_41_PAWNS)
	    MyPrintf("   index 4 vs 1 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_41);
	if(mb_info.pawn_file_type == OP_14_PAWNS)
	    MyPrintf("   index 1 vs 4 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_14);
	if(mb_info.pawn_file_type == OP_32_PAWNS)
	    MyPrintf("   index 3 vs 2 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_32);
	if(mb_info.pawn_file_type == OP_23_PAWNS)
	    MyPrintf("   index 2 vs 3 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_23);
	if(mb_info.pawn_file_type == OP_33_PAWNS)
	    MyPrintf("   index 3 vs 3 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_33);
	if(mb_info.pawn_file_type == OP_42_PAWNS)
	    MyPrintf("   index 4 vs 2 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_42);
	if(mb_info.pawn_file_type == OP_24_PAWNS)
	    MyPrintf("   index 2 vs 4 pawns, 1 opposing pair: " DEC_ZINDEX_FORMAT "\n", mb_info.index_op_24);
	for(int i = 0; i < mb_info.num_parities; i++) {
	    MyPrintf("  index %d: " DEC_ZINDEX_FORMAT "\n", i, mb_info.parity_index[i].index);
	}
    }
	...
	for(int i = 0; i < mb_info.num_parities; i++) {
		...
		if(Verbose > 4) {
		    MyPrintf("Found index " DEC_ZINDEX_FORMAT " parity index %d\n",
			ind->index, i);
		}
		break;
	    }
	...
	// if not found, check whether a blocked/opposing pawn file is applicable
	...
	if(!found_parity) {
		...
		if(mb_info.index_op_11 != ALL_ONES) {
		    ...
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == BP_11_PAWNS && fcache->pawn_file_type == BP_11_PAWNS) {
		if(mb_info.index_bp_11 != ALL_ONES) {
		    ind->index = mb_info.index_bp_11;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 1 blocking pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_21_PAWNS && fcache->pawn_file_type == OP_21_PAWNS) {
		if(mb_info.index_op_21 != ALL_ONES) {
		    ind->index = mb_info.index_op_21;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 2 vs 1 pawn, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_12_PAWNS && fcache->pawn_file_type == OP_12_PAWNS) {
		if(mb_info.index_op_12 != ALL_ONES) {
		    ind->index = mb_info.index_op_12;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 1 vs 2 pawns, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && (mb_info.pawn_file_type == OP_22_PAWNS || mb_info.pawn_file_type == DP_22_PAWNS)
		&& fcache->pawn_file_type == OP_22_PAWNS) {
		if(mb_info.index_op_22 != ALL_ONES) {
		    ind->index = mb_info.index_op_22;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 2 pawn pairs, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == DP_22_PAWNS && fcache->pawn_file_type == DP_22_PAWNS) {
		if(mb_info.index_dp_22 != ALL_ONES) {
		    ind->index = mb_info.index_dp_22;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 2 opposing pawn pairs\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_31_PAWNS && fcache->pawn_file_type == OP_31_PAWNS) {
		if(mb_info.index_op_31 != ALL_ONES) {
		    ind->index = mb_info.index_op_31;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 3 vs 1 pawn, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_13_PAWNS && fcache->pawn_file_type == OP_13_PAWNS) {
		if(mb_info.index_op_13 != ALL_ONES) {
		    ind->index = mb_info.index_op_13;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 1 vs 3 pawns, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_41_PAWNS && fcache->pawn_file_type == OP_41_PAWNS) {
		if(mb_info.index_op_41 != ALL_ONES) {
		    ind->index = mb_info.index_op_41;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 4 vs 1 pawn, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_14_PAWNS && fcache->pawn_file_type == OP_14_PAWNS) {
		if(mb_info.index_op_14 != ALL_ONES) {
		    ind->index = mb_info.index_op_14;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 1 vs 4 pawns, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_32_PAWNS && fcache->pawn_file_type == OP_32_PAWNS) {
		if(mb_info.index_op_32 != ALL_ONES) {
		    ind->index = mb_info.index_op_32;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 3 vs 2 pawn, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_23_PAWNS && fcache->pawn_file_type == OP_23_PAWNS) {
		if(mb_info.index_op_23 != ALL_ONES) {
		    ind->index = mb_info.index_op_23;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 2 vs 3 pawns, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_33_PAWNS && fcache->pawn_file_type == OP_33_PAWNS) {
		if(mb_info.index_op_33 != ALL_ONES) {
		    ind->index = mb_info.index_op_33;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 3 vs 3 pawn, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_42_PAWNS && fcache->pawn_file_type == OP_42_PAWNS) {
		if(mb_info.index_op_42 != ALL_ONES) {
		    ind->index = mb_info.index_op_42;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 4 vs 2 pawn, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }

	    if(!found_pawn_file && mb_info.pawn_file_type == OP_24_PAWNS && fcache->pawn_file_type == OP_24_PAWNS) {
		if(mb_info.index_op_24 != ALL_ONES) {
		    ind->index = mb_info.index_op_24;
		    if(Verbose > 4) {
			MyPrintf("GetMBResult: Found index " DEC_ZINDEX_FORMAT " for 2 vs 4 pawn, 1 opposing pawn pair\n", ind->index);
		    }
		    found_pawn_file = true;
		}
	    }
    ...
    if(Verbose > 3) {
	if(file_index == -1) 
	    MyPrintf("GetMBResult: file_index from scan: %d so need to open new one\n", file_index);
	else
	    MyPrintf("GetMBResult: file_index from scan: %d so can use existing one\n", file_index);
    }
	...
	int bishop_parity[2] = {NONE, NONE};
	for(int i = 0; i < mb_info.num_parities; i++) {
	    file_mb = OpenMBFile(ending, mb_info.kk_index, mb_info.parity_index[i].bishop_parity, FREE_PAWNS, side, false);
	    if(file_mb != NULL) {
		memcpy(bishop_parity, mb_info.parity_index[i].bishop_parity, sizeof(bishop_parity));
		ind->index = mb_info.parity_index[i].index;
		break;
	    }
	}
	...
	if(file_mb == NULL) {
	    if(Verbose > 3)
		MyPrintf("GetMBResult: could not find file in bishop parity search\n");
	}
	...
	if(file_mb == NULL) {
	    ...
	    if((mb_info.pawn_file_type == OP_11_PAWNS || mb_info.pawn_file_type == BP_11_PAWNS)
		&& mb_info.index_op_11 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_11_PAWNS, side, false);
		ind->index = mb_info.index_op_11;
		pawn_file_type = OP_11_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == BP_11_PAWNS && mb_info.index_bp_11 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, BP_11_PAWNS, side, false);
		ind->index = mb_info.index_bp_11;
		pawn_file_type = BP_11_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_21_PAWNS && mb_info.index_op_21 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_21_PAWNS, side, false);
		ind->index = mb_info.index_op_21;
		pawn_file_type = OP_21_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_12_PAWNS && mb_info.index_op_12 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_12_PAWNS, side, false);
		ind->index = mb_info.index_op_12;
		pawn_file_type = OP_12_PAWNS;
	    }
	    if(file_mb == NULL && (mb_info.pawn_file_type == OP_22_PAWNS || mb_info.pawn_file_type == DP_22_PAWNS)
		&& mb_info.index_op_22 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_22_PAWNS, side, false);
		ind->index = mb_info.index_op_22;
		pawn_file_type = OP_22_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == DP_22_PAWNS && mb_info.index_dp_22 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, DP_22_PAWNS, side, false);
		ind->index = mb_info.index_dp_22;
		pawn_file_type = DP_22_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_31_PAWNS && mb_info.index_op_31 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_31_PAWNS, side, false);
		ind->index = mb_info.index_op_31;
		pawn_file_type = OP_31_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_13_PAWNS && mb_info.index_op_13 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_13_PAWNS, side, false);
		ind->index = mb_info.index_op_13;
		pawn_file_type = OP_13_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_41_PAWNS && mb_info.index_op_41 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_41_PAWNS, side, false);
		ind->index = mb_info.index_op_41;
		pawn_file_type = OP_41_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_14_PAWNS && mb_info.index_op_14 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_14_PAWNS, side, false);
		ind->index = mb_info.index_op_14;
		pawn_file_type = OP_14_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_32_PAWNS && mb_info.index_op_32 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_32_PAWNS, side, false);
		ind->index = mb_info.index_op_32;
		pawn_file_type = OP_32_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_23_PAWNS && mb_info.index_op_23 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_23_PAWNS, side, false);
		ind->index = mb_info.index_op_23;
		pawn_file_type = OP_23_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_33_PAWNS && mb_info.index_op_33 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_33_PAWNS, side, false);
		ind->index = mb_info.index_op_33;
		pawn_file_type = OP_33_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_42_PAWNS && mb_info.index_op_42 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_42_PAWNS, side, false);
		ind->index = mb_info.index_op_42;
		pawn_file_type = OP_42_PAWNS;
	    }
	    if(file_mb == NULL && mb_info.pawn_file_type == OP_24_PAWNS && mb_info.index_op_24 != ALL_ONES) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_24_PAWNS, side, false);
		ind->index = mb_info.index_op_24;
		pawn_file_type = OP_24_PAWNS;
	    }
	    if(file_mb == NULL) {
		if(Verbose > 3) {
		    MyPrintf("GetMBResult: Did not find file for %s, returning file missing\n",
			OpExtensionName[mb_info.pawn_file_type]);
		}
		if(!MBFormatOnly) {
		    INDEX_DATA ind_yk;
	...
	f_read(fcache->offsets, (fcache->header.num_blocks+1)*sizeof(INDEX), fcache->fp, sizeof(HEADER));

	fcache->kk_index = mb_info.kk_index;
	memcpy(fcache->piece_type_count, mb_info.piece_type_count, sizeof(mb_info.piece_type_count));
    ...
    if(Verbose > 3) {
	MyPrintf("GetMBResult: Searching result for %s, index " DEC_ZINDEX_FORMAT " block index %lu, header block size %lu\n",
	    fcache->header.basename, ind->index, fcache->block_index, fcache->header.block_size);
    }
	...
	ULONG length = fcache->offsets[b_index + 1] - fcache->offsets[b_index];
	if(Verbose > 4) {
	    MyPrintf("GetMBResult: Reading compressed block size %lu at offset " DEC_INDEX_FORMAT " block index %lu\n",
		length, fcache->offsets[b_index], b_index);
	}
	if(length > CompressionBufferSize) {
	    if(CompressionBuffer != NULL) {
	...
	MyUncompress(fcache->block, &tmp_zone_size, CompressionBuffer, length, fcache->header.compression_method);
	...
	for(int n = 0; n < num_cached_files_high_dtz[side]; n++) {
	    ...
	    if(fcache_high_dtz->kk_index != mb_info.kk_index)
		continue;
	    if(memcmp(fcache_high_dtz->piece_type_count, mb_info.piece_type_count, sizeof(fcache_high_dtz->piece_type_count)))
		continue;
	    bool found_parity = false;
	    for(int i = 0; i < mb_info.num_parities; i++) {
		if(fcache_high_dtz->pawn_file_type != FREE_PAWNS)
	    ...
	    if(n > 0) {
		for(int i = n; i > 0; i--) {
		   cached_file_high_dtz_lru[i][side] = cached_file_high_dtz_lru[i - 1][side];
		}
	    }
	...
	if(file_index == -1) {
	    ...
	    int bishop_parity[2] = {NONE, NONE};
	    for(int i = 0; i < mb_info.num_parities; i++) {
		file_mb = OpenMBFile(ending, mb_info.kk_index, mb_info.parity_index[i].bishop_parity, 0, side, true);
		if(file_mb != NULL) {
		    memcpy(bishop_parity, mb_info.parity_index[i].bishop_parity, sizeof(bishop_parity));
		    ind->index = mb_info.parity_index[i].index;
		    break;
		...
		if((mb_info.pawn_file_type == OP_11_PAWNS || mb_info.pawn_file_type == BP_11_PAWNS) 
		    && mb_info.index_op_11 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_11_PAWNS, side, true);
		    pawn_file_type = OP_11_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == BP_11_PAWNS && mb_info.index_bp_11 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, BP_11_PAWNS, side, true);
		    pawn_file_type = BP_11_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_21_PAWNS && mb_info.index_op_21 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_21_PAWNS, side, true);
		    pawn_file_type = OP_21_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_12_PAWNS && mb_info.index_op_12 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_12_PAWNS, side, true);
		    pawn_file_type = OP_12_PAWNS;
		}
		if(file_mb == NULL && (mb_info.pawn_file_type == OP_22_PAWNS || mb_info.pawn_file_type == DP_22_PAWNS)
		    && mb_info.index_op_22 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_22_PAWNS, side, true);
		    pawn_file_type = OP_22_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == DP_22_PAWNS && mb_info.index_dp_22 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, DP_22_PAWNS, side, true);
		    pawn_file_type = DP_22_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_31_PAWNS && mb_info.index_op_31 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_31_PAWNS, side, true);
		    pawn_file_type = OP_31_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_13_PAWNS && mb_info.index_op_13 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_13_PAWNS, side, true);
		    pawn_file_type = OP_13_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_41_PAWNS && mb_info.index_op_41 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_41_PAWNS, side, true);
		    pawn_file_type = OP_41_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_14_PAWNS && mb_info.index_op_14 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_14_PAWNS, side, true);
		    pawn_file_type = OP_14_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_32_PAWNS && mb_info.index_op_32 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_32_PAWNS, side, true);
		    pawn_file_type = OP_32_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_23_PAWNS && mb_info.index_op_23 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_23_PAWNS, side, true);
		    pawn_file_type = OP_23_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_33_PAWNS && mb_info.index_op_33 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_33_PAWNS, side, true);
		    pawn_file_type = OP_33_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_42_PAWNS && mb_info.index_op_42 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_42_PAWNS, side, true);
		    pawn_file_type = OP_42_PAWNS;
		}
		if(file_mb == NULL && mb_info.pawn_file_type == OP_24_PAWNS && mb_info.index_op_24 != ALL_ONES) {
		    file_mb = OpenMBFile(ending, mb_info.kk_index, bishop_parity, OP_24_PAWNS, side, true);
		    pawn_file_type = OP_24_PAWNS;
		}
		if(file_mb == NULL)
		    return HIGH_DTZ_MISSING;
		...
		file_index = cached_file_high_dtz_lru[MAX_FILES_HIGH_DTZ - 1][side];
	    ...
	    f_read(&fcache_high_dtz->header, sizeof(HEADER), fcache_high_dtz->fp, 0);

	    if(fcache_high_dtz->header.list_element_size != sizeof(HIGH_DTZ)) {
		if(Verbose > 1) {
		    MyPrintf("GetMBResult: High DTZ size for %s, side=%s, kk_index=%d not consistent; not read\n",
			ending, ColorName(side), mb_info.kk_index);
		}
		return HIGH_DTZ_MISSING;
	    }

	    if(fcache_high_dtz->header.num_blocks > fcache_high_dtz->max_num_blocks) {
		if(fcache_high_dtz->max_num_blocks > 0) {
		    MyFree(fcache_high_dtz->offsets, (fcache_high_dtz->max_num_blocks + 1) * sizeof(INDEX));
		    MyFree(fcache_high_dtz->starting_index, (fcache_high_dtz->max_num_blocks + 1) * sizeof(ZINDEX));
		}
		fcache_high_dtz->max_num_blocks = fcache_high_dtz->header.num_blocks;
		fcache_high_dtz->offsets = (INDEX *)MyMalloc((fcache_high_dtz->max_num_blocks + 1)*sizeof(INDEX));
		fcache_high_dtz->starting_index = (ZINDEX *)MyMalloc((fcache_high_dtz->max_num_blocks + 1)*sizeof(ZINDEX));
	    }

	    INDEX l_offset = sizeof(HEADER);
	    f_read(fcache_high_dtz->offsets, (fcache_high_dtz->header.num_blocks+1)*sizeof(INDEX), 
		fcache_high_dtz->fp, l_offset);
	    l_offset += (fcache_high_dtz->header.num_blocks + 1) * sizeof(INDEX);
	    f_read(fcache_high_dtz->starting_index, (fcache_high_dtz->header.num_blocks + 1) * sizeof(ZINDEX),
		fcache_high_dtz->fp, l_offset);
		...
		if(fcache_high_dtz->max_block_size > 0) {
		    MyFree(fcache_high_dtz->block, fcache_high_dtz->max_block_size);
		}
	    ...
	    memcpy(fcache_high_dtz->piece_type_count, mb_info.piece_type_count, sizeof(mb_info.piece_type_count));
	    memcpy(fcache_high_dtz->bishop_parity, bishop_parity, sizeof(bishop_parity));
	...
	// if index is outside range of all indices with depth > 254, depth is 254
	if(ind->index < fcache_high_dtz->starting_index[0] || 
	   ind->index > fcache_high_dtz->starting_index[fcache_high_dtz->header.num_blocks]) {
	    if(cache_hit)
		CacheHits++;
	    else
		DBHits++;
	...
	n_per_block = fcache_high_dtz->header.block_size / fcache_high_dtz->header.list_element_size;
	...
	if(fcache_high_dtz->block_index != -1) {
	    ...
	    if(ind->index < hptr[0].index || ind->index > hptr[n_per_block_cached - 1].index)
		fcache_high_dtz->block_index = -1;
	}
	...
	if(fcache_high_dtz->block_index == -1) {
	    cache_hit = false;
	    // do binary search to find which block of indices the depth would be
	    ...
	    if((l == fcache_high_dtz->header.num_blocks) || (fcache_high_dtz->starting_index[l] > ind->index))
		l--;
	    // now read block from file
	    fcache_high_dtz->block_index = l;
	    ULONG length = fcache_high_dtz->offsets[fcache_high_dtz->block_index + 1] -
	    ...
	    f_read(CompressionBuffer, length, fcache_high_dtz->fp, fcache_high_dtz->offsets[fcache_high_dtz->block_index]);
	    ULONG n_per_block_cached = n_per_block;
	    if(fcache_high_dtz->block_index == fcache_high_dtz->header.num_blocks - 1) {
		ULONG rem = fcache_high_dtz->header.n_elements % n_per_block;
		if(rem != 0)
	    ...
	    MyUncompress((BYTE *)fcache_high_dtz->block, &tmp_zone_size, CompressionBuffer,
		length, fcache_high_dtz->header.compression_method);
	...
	HIGH_DTZ *kptr = (HIGH_DTZ *)bsearch(&key, fcache_high_dtz->block, n_per_block_cached,
					sizeof(HIGH_DTZ), high_dtz_compar);
...
static int
ScorePosition(BOARD *BoardIn, INDEX_DATA *index)
{
    if(Verbose > 4) {
	...
	if((Board.side == WHITE) || 
	    result == LOST || result == WON || result == HIGH_DTZ_MISSING) {
	    if(Verbose > 3) {
    ...
    if(Verbose > 3) {
	char label[64];
	sprintf(label, "ScorePosition: result=%d result_flipped=%d",result,result_flipped);
	DisplayBoard(&Board, label);
    }

    if(result_flipped == WON || result_flipped == LOST || result_flipped == HIGH_DTZ_MISSING)
	...
	if(Verbose > 1) {
	    char label[64];
	    sprintf(label,"ScorePosition: Unexpected result=%d, flipped=%d combination",
                         ...
                         bool skip_pawn_promotion, bool try_all_promos)
{
    ...
    int nmoves = GenLegalMoves(Board, move_list, skip_pawn_promotion, try_all_promos);
	...
	if(xside == BLACK && move_list[i].score != UNKNOWN && move_list[i].score != UNRESOLVED &&
	   move_list[i].score > 0 && move_list[i].score != HIGH_DTZ_MISSING)
...
static void
PrintPGNHeader(BOARD *Board, int score, char *result)
{
...
static void
FindBestLine(BOARD *Board, bool mark_mzugs, bool mark_best, bool mark_squeeze)
{
    ...
    bool in_check, flip = false, mzug = false, repeated_pos = false, capture_seen = false;
    ...
    if(score == UNKNOWN || score == ILLEGAL ||
	...
	if(xscore == DRAW || xscore == NOT_WON || xscore == LOST || xscore <= 0) {
	    mzug = true;
	}
	Board->side = OtherSide(Board->side);
    }
...
final_move:
	...
	if(score > 0 && score != LOST) {
	    good_moves = 0; best_index = 0; best_move = -16384;
...
//		fprintf(stderr,"MSB: nreplies %d, in_check %d\n", nreplies, in_check);
		...
		if(xscore == DRAW ||
		   xscore == NOT_WON || xscore == NOT_LOST ||
...
//		    fprintf(stderr,"MSB: for xscore=%d, continue\n", xscore);
		...
		if(score == 1) {
			...
			(metric == DTZ && abs(move_list[i].piece_moved) == PAWN)) {
			...
			if(best_move < 0) {
			    ...
			    if(move_list[i].flag & (CAPTURE|PROMOTION)) {
				int material = abs(move_list[i].piece_captured) +
			    ...
			    else if(metric == DTZ && abs(move_list[i].piece_moved) == PAWN) {
				best_index = i;
			    }
			}
		    }
		    ...
		    bool pchange = (move_list[i].flag & (CAPTURE|PROMOTION)) ||
				   (metric == DTZ && abs(move_list[i].piece_moved) == PAWN);
//		    fprintf(stderr,"MSB: pchange=%d\n", pchange);
//		    fprintf(stderr,"MSB: xscore=%d, -(score - 1)=%d, best_move=%d\n", xscore, -(score - 1), best_move);
		    if(!pchange && xscore == -(score - 1) && best_move != xscore) {
			best_move = xscore;
			best_index = i;
//			fprintf(stderr,"MSB: changing best_move to %d\n", best_move);
		    }
//		    else {
...
//		    }
		...
		if(score == 0) {
		...
		else if(score == -1 && !(move_list[i].flag & (CAPTURE|PROMOTION))) {
		    Move reply_list[MAX_MOVES];
		    char mstring[64];
		    int nreplies, best_index_x = 0, best_move_x = -16384, biggest_capture = 0;
		    MakeMove(Board, &move_list[i]);
		    if(flip) {
			FlipBoard(Board);
		    }	
			...
			nreplies_x = GenLegalMoves(Board, reply_list_x, true, true);
			in_check = IsInCheck(Board, Board->side);
			...
			if(xscore_x == UNKNOWN) {
			    if(Verbose > 2) {
				GetMoveString(&reply_list[j], move_string, false);
				MyPrintf("Move %s not resolved\n",move_string);
			    }
			    continue;
			}
			...
			if(xscore_x == DRAW ||
			    xscore_x == NOT_WON || xscore_x == NOT_LOST || xscore_x == LOST ||
			    xscore_x == UNRESOLVED || xscore_x == ILLEGAL) {
			    continue;
			}

			if((reply_list[j].flag & (CAPTURE|PROMOTION)) ||
			    (metric == DTZ && abs(reply_list[j].piece_moved) == PAWN)) {
			...
			if(xscore_x == 0) {
				...
				if(reply_list[j].flag & (CAPTURE|PROMOTION)) {
				    int material = abs(reply_list[j].piece_captured) +
				...
				else if(metric == DTZ && abs(reply_list[j].piece_moved) == PAWN) {
				    best_index_x = j;
				}
			    }
			}
...
show_move:
	...
	GetShortMoveString(&move_list[best_index], move_list,
	                   nmoves, move_string);

	if(move_list[best_index].flag & (CAPTURE|PROMOTION))
	    capture_seen = true;
	phase_change_seen = capture_seen;
	if((metric == DTZ || StopAtPawnMove) && abs(move_list[best_index].piece_moved) == PAWN)
	...
	if(!in_check && mark_mzugs && (score == LOST || score <= 0)) {
	    Board->side = OtherSide(Board->side);
	    int xscore = ScorePosition(Board, &index);
	    if(xscore == NOT_WON || xscore == DRAW || xscore == LOST || xscore <= 0)
		mzug = true;
	    Board->side = OtherSide(Board->side);
	}
    }
...
static void
EvaluateEnding(BOARD *Board, bool other_side, bool best_move)
{
    int score, xscore;
	...
	sprintf(&score_str[strlen(score_str)],"; %s %s",
	        SideTm(Board->side), xscore_str);
   	Board->side = OtherSide(Board->side); 
    }
	...
	if(Board->side == BLACK &&
	    (score == NOT_WON || score == WON || (score > 0 && score != DRAW && score != NOT_LOST && score != LOST)))
	    flip = true;
...
static void PrintHelpScreen()
{
    ...
    "            (hitting ENTER is equivalent to entering 0, the best move)\n"
...
repeat:
	...
	for(i = 0; i < nmoves; i++) {
	    MyPrintf("   from=%02o to=%02o score=%d\n",move_list[i].from,move_list[i].to,move_list[i].score);
	}
    }
    ...
    MyPrintf("\nPosition[%d], Ply %d, KK %d, Offset " DEC_ZINDEX_FORMAT,
                npos, curr_stack, index.kk_index, index.index);
    DisplayBoard(&Board, label);
...
prompt:
    memset(line, 0, sizeof(line));
    MyPrintf("\n[help, [#], +, -, m [#], on|off, f, n, p, g #, \"title\", q[uit]]) ");
    MyFlush();
    do {
	fgets(line, MAX_LINE-1, stdin);
    } while(line[0] == 0);
	...
	if(nply < curr_stack) {
	    ...
	    int ns;
	    if(max_stack < nply) nply = max_stack;
	    ns = nply - curr_stack;
	    while(--ns >= 0) {
		MakeMove(&Board, &move_stack[curr_stack++]);
	    }
	...
	if(move_no >= 0) {
		...
		memmove(&move_stack[0], &move_stack[1], (MAX_STACK-1)*sizeof(Move));
		curr_stack--;
	    }
	    memcpy(&move_stack[curr_stack++], &move_list[move_no], sizeof(Move));
...
static void EvaluateBleicher(char *pos, int side_in)
{
    ...
    if(side == WHITE &&
       (score == NOT_LOST || score == LOST || score <= 0))
       flipped = true;
	...
	if((mptr->flag & PROMOTION) &&
	   (abs(mptr->piece_promoted) == PAWN))
	    continue;
	...
	if(mptr->score == UNKNOWN || mptr->score == NOT_WON || mptr->score == NOT_LOST)
	    MyPrintf("%d\n",BLEICHER_NOT_FOUND);
...
static void PrintSummary()
{
    MyPrintf("Fopen: %lu   Fclose: %lu"
             "   Fread: " DEC_INDEX_FORMAT
             "   Fwrite: " DEC_INDEX_FORMAT "\n",
...
PGNToken
ParseTagList(PGNToken symbol, FILE *fp, ParseType *yylval, BOARD *start_pos)
{
	...
	if(symbol == STRING) {
	    ...
	    */
	    if((!CheckSyntaxOnly && !PositionDatabase && !PrintBadlyPlayedPositions) || Verbose > 3) {
		char output[512];
		if(tag_index == ANNOTATOR_TAG) {
		    AnnotatorSeen = true;
		    AnnotatorLine = NumOutputLines;
		}
		sprintf(output,"[%s \"%s\"]",TagList[tag_index], yylval->string_info);
		if(Verbose > 3)
...
static int
ProcessPosition(BOARD *pos, bool evaluate, bool best_capture, bool mark_mzugs)
{
    MB_INFO mb_info;
    char ending[16];
    int changed = 0;
	...
	if(score == UNKNOWN || score == NOT_WON || score == NOT_LOST) {
		...
		for(int i = 0; i < ncaptures; i++) {		
		    ...
		    if(tmp_score == UNKNOWN || tmp_score == UNRESOLVED || 
		       tmp_score == ILLEGAL || tmp_score == LOST || tmp_score == NOT_WON)
			continue;
		    if(tmp_score == STALE_MATE || tmp_score == DRAW || tmp_score == NOT_LOST)
			score_new = NOT_LOST;
		...
		if((score == NOT_WON && score_new == NOT_LOST) ||
		   ...
		   (score_new != UNKNOWN && (score == NOT_WON || score == NOT_LOST))) {
		    score = score_new;
		    changed = 1;
		}
		pos->score = score;
	...
	if(zz_type == UNKNOWN) {
	    if(score == WON || 
		(score > 0 && score != DRAW && score != NOT_WON && score != LOST && score != UNKNOWN) ||
    ...
    if(FlagRestrictedPromotions && mb_info.pawn_file_type == FREE_PAWNS && pos->num_pieces == 8) {
	RESTRICTED_PROMOTION *rptr, ref;
	strcpy(ref.ending, ending);
	rptr = (RESTRICTED_PROMOTION *)bsearch(&ref, RestrictedPromotionList, NumRestrictedPromotionEndings, 
...
static int
EvaluatePosition(BOARD *board, bool best_capture, bool mark_mzugs)
{
    return ProcessPosition(board, true, best_capture, mark_mzugs);
}

PGNToken
ParseMoveList(PGNToken symbol, FILE *fin, ParseType *yylval, 
              BOARD *prev_pos, BOARD *curr_pos)
{
	...
	if(symbol == MOVE) {
	    ...
	    npseudo_moves = GenPseudoLegalMoves(curr_pos, pseudo_move_list, true);
	    nmoves = GenLegalFromPseudoMoves(curr_pos, move_list, pseudo_move_list, npseudo_moves);
	    
	    if(Verbose > 5) {
	        MyPrintf("# pseudo-legal: %d,  legal: %d\n", npseudo_moves, nmoves);
	    }
		...
		GetShortMoveString(&move, pseudo_move_list, npseudo_moves, move_string);
		OutputPGNStringCashed(move_string);
	    ...
	    if(CheckSyntaxOnly && (RAV_level == 0 || AnnotateVariations) && is_right_number_of_pieces) {
		num_positions++;
	    ...
	    if((InsertComments || AddEGTBComments || PrintBadlyPlayedPositions) && (RAV_level == 0 || AnnotateVariations)) {
		bool phase_change = false;
		if(memcmp(curr_pos->piece_type_count, prev_pos->piece_type_count, sizeof(curr_pos->piece_type_count)))
		     phase_change = true;
		int move_score = EvaluateMove(prev_pos->score, curr_pos->score, phase_change);
		if((phase_change || !EGTBWritten) && curr_pos->score != UNKNOWN) {
		    if(InsertComments && is_right_number_of_pieces) {
			char score_string[16];
			...
			if(curr_pos->promos != QRBN_PROMOTIONS && curr_pos->promos != 0) {
				...
				promos[n++] = 'n';
			    promos[n] = '\0';
			    sprintf(comment,"%s_%s_%d (%s)", Annotator, promos, curr_pos->num_pieces, score_string);
			}
			else 
			    sprintf(comment,"%s_%d (%s)", Annotator, curr_pos->num_pieces, score_string);
			OutputPGNCommentCashed(comment);
			EGTBWritten = true;
			ContainsEGTBEvaluation = true;
		    }
		}
		if(IsResultChangingMove(move_score) || (DepthDelta > 0 && abs(move_score) >= DepthDelta)) {
			...
			if(curr_pos->promos != QRBN_PROMOTIONS && curr_pos->promos != 0) {
				...
				promos[n++] = 'n';
			    promos[n] = '\0';
			    sprintf(comment,"%s_%s_%d %s (%s)", Annotator, promos, prev_pos->num_pieces, move_string, score_string);
			}
			else
			    sprintf(comment,"%s_%d %s (%s)", Annotator, prev_pos->num_pieces, move_string, score_string);
			OutputPGNCommentCashed(comment);
		    }
			...
			if(IsResultChangingMove(move_score)) {
				...
				ScoreListPGN[NumBadMovesPGN].move_no = prev_pos->full_move;
				ScoreListPGN[NumBadMovesPGN].side = prev_pos->side;
				NumBadMovesPGN++;
			    }
			}
			else {
			    if(NumDeltaMovesPGN < MAX_GAME_MOVES) {
				ScoreListDeltaPGN[NumDeltaMovesPGN].score = move_score;
				ScoreListDeltaPGN[NumDeltaMovesPGN].move_no = prev_pos->full_move;
				ScoreListDeltaPGN[NumDeltaMovesPGN].side = prev_pos->side;
				NumDeltaMovesPGN++;
			    }
			}
		    }
		    else {
			if(IsResultChangingMove(move_score) && is_prior_right_number_of_pieces) {
		...
		if(symbol == MOVE || symbol == MOVE_NUMBER) {
		    ...
		    symbol = ParseMoveList(symbol, fin, yylval, &new_prev_pos, &new_curr_pos);
		}
		else {
		    fprintf(stderr,"Variation not followed by move in game %u\n", num_games);
		    FlushPGNOutputCashed();
		    exit(1);
		}

		while(symbol != RAV_END) {
		    fprintf(stderr,"Unexpected  %s in input, expected end of variation\n", yylval->string_info);
		    symbol = GetNextToken(fin, yylval);
		    FlushPGNOutputCashed();
		}
...
PGNToken
ParseGame(PGNToken symbol, FILE *fin, ParseType *yylval, BOARD *curr_pos, 
          bool best_capture)
{
    ...
    legal = ReadPosition("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", curr_pos, NULL);
	...
	if((InsertComments || AddEGTBComments) && is_right_number_of_pieces) {
		...
		if(curr_pos->side == BLACK) sip1 = WON;
		else sip1 = LOST;
	    }
	    else if(curr_pos->result == LOST) {
		if(curr_pos->side == WHITE) sip1 = WON;
		else sip1 = LOST;
	    }
		...
		if(InsertComments) {
			...
			if(curr_pos->promos & (1 << (KNIGHT)))
			    promos[n++] = 'n';
			promos[n] = '\0';
			sprintf(comment,"%s_%s_%d (%s)", Annotator, promos, curr_pos->num_pieces, move_string);
		    }
		    else 
			sprintf(comment,"%s_%d (%s)", Annotator, curr_pos->num_pieces, move_string);
		    OutputPGNCommentCashed(comment);
		}
		if(NumBadMovesPGN < MAX_GAME_MOVES) {
...
static void Usage(char *prog_name, bool show_version)
{
    ...
    " -p[elprsSz!] [l] - show best line. For -pz, also label mzugs with {zz}:\n"
    ...
    "                    for r, mark repeats of the original position with switched sides with {rr}\n"
    "                    for e, use EG format ('S' for 'N', no space after '.')\n"
    "                    for s, stop list after transition to capture sub game (or pawn move with S)\n"
    " -a[v][options]     process PGN pos_file (for v parse variations as well). Valid options:\n"
    "     s            - check syntax only and provide positions stats\n"
    "     h[m-n]       - create position file (for later reading with -o)\n"
    "                    m-n number of pieces between m and n (default %d and %d)\n"
    "     [aioc]       - create PGN with comments, based on data from -o file\n"
    "                    for a, add annotator to PGN (either EGTB or from -t)\n"
    "                    for o, add position valuation summary to header from -o file\n"
    "                    for i, insert scores from db file from -o file\n"
    "                    for c, only include games with at least one TB position\n"
    "     e            - print EPD with incorrectly played positions in pos_file\n"
    ...
    " -o file          - read position evaluation from file for -ao option\n"
    ...
    "                    for o, also add position summary to header in -ao option\n"
    " -m[p[u]]         - find best move (for p, output in PGN rather than EGTB format)\n"
    ...
    " -ul file         - read endings with restricted promotions from file\n"
    " -u[x] promo      - allowed promotions (q, r, b, n, qn,...; default qrbn)\n"
    "                    ux: use all promotions for endings with < 8 pieces\n"
    " -n               - exclude en passant\n"
    " -c[f]            - include castling rights (will return unknown from egtb)\n"
    "                    (for cf, include Chess960 castling)\n"
    " -w n             - use output line width n (for -a and -p options), default %d\n"
...
int main(int argc, char *argv[])
{
    ...
    bool best_line = false, mark_mzugs = false, mark_squeeze = false, query = false;
...
#   endif
	...
	if(strcmp(token, "quit") == 0)
		...
		if(score == DRAW || score == STALE_MATE)
		    ...
		    sprintf(score_str,"%+d", score);
		printf("Position in YKMB database, kk_index=%d offset= " DEC_ZINDEX_FORMAT " score=%s\n",
		...
		if(nreplies == 0) {
		    ...
		    if((move_list[i].flag & (CAPTURE | PROMOTION)) && IsWinningScore(move_list[i].score))
			move_list[i].score = 1;
		}
	    }
		...
		if(move_list[i].score == DRAW)
		    ...
		    sprintf(score_str, "cp -10000");
		else if(move_list[i].score == UNKNOWN || move_list[i].score == UNRESOLVED)
...
#if defined(UCI_LOG)
		fprintf(fout,"info depth 1 seldepth 1 multipv %d time %d score %s nodes %lu nps %lu tbhits %lu pv %s\n",
		    i+1, time_taken, score_str, nodes, nps, DBHits, move);
#endif

		printf("info depth 1 seldepth 1 multipv %d time %d score %s nodes %lu npos %lu tbhits %lu pv %s\n",
		    i+1, time_taken, score_str, nodes, nps, DBHits, move);
	    }
	...
	if(argv[iarg][0] == '-') {
		...
		case 'A':
			comment_games = true;
			if(strchr(&(argv[iarg][2]),'v') || strchr(&(argv[iarg][2]),'V'))
			    AnnotateVariations = true;
			if(strchr(&(argv[iarg][2]),'s') || strchr(&(argv[iarg][2]),'S'))
			    CheckSyntaxOnly = true;
			if(strchr(&(argv[iarg][2]),'c') || strchr(&(argv[iarg][2]),'C'))
			    best_capture = true;
			if(strchr(&(argv[iarg][2]),'o') || strchr(&(argv[iarg][2]),'O'))
			    AddEGTBComments = true;
			if(strchr(&(argv[iarg][2]),'h') || strchr(&(argv[iarg][2]),'H'))
			    PositionDatabase = true;
			if(strchr(&(argv[iarg][2]),'i') || strchr(&(argv[iarg][2]),'I'))
			    InsertComments = true;
			if(strchr(&(argv[iarg][2]),'a') || strchr(&(argv[iarg][2]),'A'))
			    AddAnnotator = true;
			if(strchr(&(argv[iarg][2]),'e') || strchr(&(argv[iarg][2]),'E'))
			...
			if(sptr != NULL) {
				...
				fprintf(stderr,"Could not read minimum number of pieces\n");
				exit(1);
			    }
			    sptr = strchr(sptr, '-');
			    if(sptr == NULL) {
				fprintf(stderr,"Internal error parsing maximum number of pieces\n");
				exit(1);
			    }
			    sptr++;
			    if(sscanf(sptr,"%d", &MaximumNumberOfPieces) != 1) {
				fprintf(stderr,"Could not read maximum number of pieces\n");
				exit(1);
			    }
			}
			break;
		...
		case 'M':
			if((argv[iarg][2] == 'b') || (argv[iarg][2] == 'B')) {
			    ...
			    best_move = true;
			    if(strchr(&(argv[iarg][2]),'p') || strchr(&(argv[iarg][2]),'P'))
				OutputPGNFormat = true;
			    if(strchr(&(argv[iarg][2]),'u') || strchr(&(argv[iarg][2]),'U'))
		...
		case 'H':
			...
			if(strchr(&(argv[iarg][2]),'s') || strchr(&(argv[iarg][2]),'S'))
			    sort_list = true;
			if(strchr(&(argv[iarg][2]),'z') || strchr(&(argv[iarg][2]),'Z'))
			    mark_mzugs = true;
			if(strchr(&(argv[iarg][2]),'r') || strchr(&(argv[iarg][2]),'R'))
			    mark_cz_zugs = true;
			if(strchr(&(argv[iarg][2]),'y') || strchr(&(argv[iarg][2]),'Y'))
			    convert_from_mb_to_yk = true;
			if(strchr(&(argv[iarg][2]),'m') || strchr(&(argv[iarg][2]),'M'))
		...
		case 'P':
			...
			if(strchr(&(argv[iarg][2]),'l') || strchr(&(argv[iarg][2]),'L')) {
			    if(iarg + 1 < argc) {
				if(sscanf(argv[iarg+1],"%d", &MaxLineLength) != 1) {
				   fprintf(stderr,"Could not read maximum line length from %s\n", argv[iarg+1]);
				   goto error;
				}
				...
				fprintf(stderr,"Need maximum length as additional argument for %s\n", argv[iarg]);
				goto error;
			    }
			}
			break;
		...
		case 'C':
			IgnoreCastle = false;
			if(strchr(&(argv[iarg][2]),'f') || strchr(&(argv[iarg][2]),'F'))
			   Chess960 = true;
			break;
		case 'l':
		case 'L':
			if(iarg + 1 < argc) {
			    flog = fopen(argv[iarg+1],
			            argv[iarg][2] == 'a' ? "a" : "w");
			    if(flog == NULL) {
		...
		case '?':
			...
			exit(0);
		default:
error:		MyPrintf("Illegal option '%s'\n",argv[iarg]);
		Usage(argv[0], false);
		exit(1);
	    }
	    memmove(argv+iarg, argv+iarg+1, (argc-iarg) * sizeof(char *));
	...
	for(int i = 0; i < 2; i++) {
		...
		if(n < NumRestrictedPromotionEndings) {
		    ...
		    strncpy(RestrictedPromotionList[n].ending, ending, sizeof(RestrictedPromotionList[n].ending));
		...
		rewind(fpromos);
		if(NumRestrictedPromotionEndings > 0) {
		    RestrictedPromotionList = 
			(RESTRICTED_PROMOTION *)MyMalloc(NumRestrictedPromotionEndings*sizeof(RESTRICTED_PROMOTION));
		}
	    }
	}
	...
	if(NumRestrictedPromotionEndings > 0) {
	    qsort(RestrictedPromotionList, NumRestrictedPromotionEndings, sizeof(RESTRICTED_PROMOTION), restricted_promo_compar);
	    int n_total = 1;
	    for(int i = 1; i < NumRestrictedPromotionEndings; i++) {
		if(restricted_promo_compar(&RestrictedPromotionList[i], &RestrictedPromotionList[n_total-1]) != 0) {
		    memcpy(&RestrictedPromotionList[n_total], &RestrictedPromotionList[i], sizeof(RestrictedPromotionList[0]));
		    n_total++;
		}
	    }
	...
	for(int i = 0; i < 2; i++) {
		...
		if(nscores < NumDBPositions) {
		    strncpy(PositionDB[nscores].ending, pos_data.ending, sizeof(PositionDB[nscores].ending));
		...
		rewind(fdb);
		if(NumDBPositions > 0) {
		    PositionDB = (POSITION_DB *)MyMalloc(NumDBPositions*sizeof(POSITION_DB));
		}
	    }
	}
	fclose(fdb);
	...
	if(NumDBPositions > 0) {
	    qsort(PositionDB, NumDBPositions, sizeof(POSITION_DB), db_pos_compar);
	    int n_total = 1;
	    for(int i = 1; i < NumDBPositions; i++) {
		if(db_pos_compar(&PositionDB[i], &PositionDB[n_total-1]) != 0) {
		    memcpy(&PositionDB[n_total], &PositionDB[i], sizeof(PositionDB[0]));
		    n_total++;
		}
		else {
		    int best_score = PositionDB[n_total-1].score;
		    if(best_score == NOT_WON) {
			if(PositionDB[i].score == DRAW || PositionDB[i].score <= 0)
			    best_score = PositionDB[i].score;
		    }
		    else if(best_score == NOT_LOST) {
			if(PositionDB[i].score > 0 && PositionDB[i].score != UNKNOWN && PositionDB[i].score != NOT_WON)
			    best_score = PositionDB[i].score;
		    }
	...
	/* find number of scores (skipping unknowns and variations) and read them into list */
	...
	for(int i = 0; i < 2; i++) {
		...
		if(score == UNKNOWN && (zz_type == UNKNOWN || zz_type == NO_MZUG) && cz_type == '0')
		    continue;

		if(i == 0) {
		    if(score != UNKNOWN)
			RawScores++;
		    if((zz_type != UNKNOWN && zz_type != NO_MZUG) || cz_type != '0')
			NumZZPositions++;
		    continue;
		}
		...
		rewind(fscore);
		if(RawScores > 0) {
		    ScoreList = (SCORE *)MyMalloc((RawScores+1)*sizeof(SCORE));
		    if(DepthDelta > 0)
			ScoreListDelta = (SCORE *)MyMalloc((RawScores+1)*sizeof(SCORE));
		}
		if(NumZZPositions > 0) {
		    ZZList = (SCORE *)MyMalloc((NumZZPositions+1)*sizeof(SCORE));
		}		
	    }
	}
	fclose(fscore);
	...
	if(RawScores > 0) {
		...
		if((ending_list = (ENDING_LIST *)MyMalloc(num_distinct_endings * sizeof(ENDING_LIST))) == NULL) {
		    fprintf(stderr,"Could not allocate list of %d endings, statistics ignored\n ", num_distinct_endings);
		...
		if(extra_statistics) {
			...
			if(strcmp(ScoreList[i-1].ending, ScoreList[i].ending)) {
				...
				fprintf(stderr,"Warning: more than the %d expected number of distinct endings\n",
				    num_distinct_endings);
				continue;
			    }
			    strcpy(ending_list[n_endings++].ending, ScoreList[i].ending);
			}
		    }
		    if(n_endings < num_distinct_endings) {
			fprintf(stderr,"Warning: only found %d distinct endings, expected %d\n",
	    ...
	    if(DepthDelta > 0)
		memcpy(ScoreListDelta, ScoreList, (RawScores+1)*sizeof(SCORE));
	}
	...
	for(int i = 0; i < RawScores; i++) {
	   ...
	   if(ScoreList[i].game_num == ScoreList[i+1].game_num) {
		...
		if(side == ScoreList[i+1].side)
		    continue;
		if((side == WHITE && (ScoreList[i+1].move_no != ScoreList[i].move_no)) ||
		   (side == BLACK && (ScoreList[i+1].move_no != ScoreList[i].move_no + 1)))
		    continue;
		if(strcmp(ScoreList[i+1].ending, ScoreList[i].ending))
		    phase_change = true;
		...
		if(result == DRAW)
		   ...
		   if(side == WHITE) sip1 = WON;
		   else            sip1 = LOST;
		}
		else if(result == LOST) {
		    if(side == BLACK) sip1 = WON;
		    else            sip1 = LOST;
		}
	   ...
	   if(DepthDelta > 0) {
		strcpy(ScoreListDelta[NumBadMovesDelta].ending, ScoreListDelta[i].ending);
		ScoreListDelta[NumBadMovesDelta].game_num = ScoreListDelta[i].game_num;
	   ...
	   if(extra_statistics) {
		...
		ref_ptr = (ENDING_LIST *)bsearch(&ref, ending_list, num_distinct_endings, sizeof(ENDING_LIST), ending_list_compar);
		if(ref_ptr != NULL)
		    ref_ptr->num_total++;
	   }
	...
	if(Verbose > 0) {
	    ...
	    if(DepthDelta > 0)
		MyPrintf("Found %d large delta moves in %s\n", NumBadMovesDelta, ScoreFile);
	    MyFlush();
	}

	if(extra_statistics) {
	    qsort(ending_list, num_distinct_endings, sizeof(ending_list[0]), move_count_compar);
	    MyPrintf("%12s  %10s  %10s  %s\n",
		"Ending", "Num-Moves", "Bad-Moves", "% Bad");
	    for(int i = 0; i < num_distinct_endings; i++) {
		if(ending_list[i].num_total > 0) {
		    MyPrintf("%12s  %10u  %10u  %5.2lf\n",
			ending_list[i].ending, ending_list[i].num_total, ending_list[i].num_bad, 
			   100.0*(double)ending_list[i].num_bad / (double)ending_list[i].num_total);
		}
	    }
	    MyPrintf("\n");
	}
	...
	if(show_bad_games) {
		...
		MyPrintf("Ending %10s Game %8d move %3d %c %s\n", 
			ScoreList[i].ending, ScoreList[i].game_num, ScoreList[i].move_no, 
	...
	if((yylval.string_info = (char *)malloc(MAX_PGN_LINE * sizeof(char)))==NULL) {
	...
	legal = ReadPosition("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", &current_pos, NULL);
	...
	while(symbol != EOF_TOKEN) {
	    skipping_game = false;
	    symbol = ParseGame(symbol, fin, &yylval, &current_pos, best_capture);
	    if(symbol != EOF_TOKEN) {
	   	num_games++;
		if(!CheckSyntaxOnly && !PositionDatabase && !PrintBadlyPlayedPositions)
		    MyPrintf("\n");
	...
	for(;;) {
		...
		if(side != NEUTRAL) {
		    if(best_line)
			FindBestLine(&Board, mark_mzugs, mark_best, mark_squeeze);
		    else
	...
	*sptr = '\0';
	...
	MyPrintf("Cannot convert both from MB to YK and from YK to MB, ignored\n");
    ...
    if(process_positions && (sort_list || mark_cz_zugs) && !(convert_from_yk_to_mb || convert_from_mb_to_yk)) {
	POSITION *pos_list;
	while(fgets(line,MAX_LINE-1,fin) != NULL) {
	    POSITION_DATA pos_data;		
	...
	while(fgets(line,MAX_LINE-1,fin) != NULL) {
	    ...
	    if(!mark_cz_zugs) {
		for(int i = KING-1; i >= 0; i--) {
		    if(Board.piece_type_count[0][i] < Board.piece_type_count[1][i]) {
			pos_list[pos_count].flipped = true;
			break;
		    }
		    else if(Board.piece_type_count[0][i] > Board.piece_type_count[1][i])
			break;
		}
	    ...
	    memcpy(pos_list[pos_count].piece_type_count,
	           Board.piece_type_count,
	...
	if(mark_cz_zugs) {
	    ...
	    int npos = 1;
	    memcpy(ginfo[0].piece_type_count, pos_list[0].piece_type_count, sizeof(ginfo[0].piece_type_count));
	    int cz_index = 0;
	    for(int i = 1; i < pos_count; i++) {
		if(i == (pos_count - 1) || pos_list[i].game_num != pos_list[i-1].game_num) {
			...
			for(int k = j+1; k < npos; k++) {
			    ...
			    if(ginfo[j].kk_index == ginfo[k].kk_index && ginfo[j].offset == ginfo[k].offset &&
			      ((res1 == WON && res2 == WON && ply1 == 0 && ply2 == 1) ||
			       (res1 == LOST && res2 == LOST && ply1 == 1 && ply2 == 0)) &&
				!memcmp(ginfo[j].piece_type_count, ginfo[k].piece_type_count, sizeof(ginfo[j].piece_type_count))) {
				pos_list[ginfo[j].pindex].cz_type = 'a' + cz_index;
				pos_list[ginfo[k].pindex].cz_type = 'A' + cz_index;
				pos_list[ginfo[k].pindex].cz_depth = ginfo[k].pindex - ginfo[j].pindex;
				cz_index++;
				break;
			    }
			}
		...
		if(npos >= MAX_PLY) {
		    fprintf(stderr,"Length of game > %d ply, need to increase MAX_PLY\n", MAX_PLY);
		    exit(1);
		}
		ginfo[npos].kk_index = pos_list[i].kk_index;
		ginfo[npos].offset = pos_list[i].offset;
		ginfo[npos].result = isupper(pos_list[i].result) ? tolower(pos_list[i].result) : pos_list[i].result;
		ginfo[npos].pindex = i;
		if(pos_list[i].side == WHITE)
		    ginfo[npos].ply = 2*pos_list[i].move_no - 2;
		else
		    ginfo[npos].ply = 2*pos_list[i].move_no - 1;
		memcpy(ginfo[npos].piece_type_count, pos_list[i].piece_type_count, sizeof(ginfo[npos].piece_type_count));
		npos++;
	    }
		...
		char ending[MAX_LINE], score_string[MAX_LINE], zz_string[MAX_LINE];    
	...
	if(convert_from_mb_to_yk) {
		...
		if(!single_pos || pos_count == npos) {
			...
			num_studies += EvaluatePosition(&Board, best_capture, mark_mzugs);
		    else 
			EvaluateEnding(&Board, other_side, best_move);
		    if(single_pos)
			break;
