...
fn strength(board: &Board, color: Color) -> i32 {
    ...
    (side & board.pawns()).count() as i32
        + (side & board.knights()).count() as i32 * 3
        + (side & board.bishops()).count() as i32 * 3
        + (side & board.rooks()).count() as i32 * 5
        + (side & board.queens()).count() as i32 * 9
}

fn material_balance(pos: &Chess, depth: u32) -> i32 {
    if depth == 0 {
        strength(pos.board(), pos.turn()) - strength(pos.board(), !pos.turn())
    } else {
        pos.legal_moves()
            .into_iter()
            .map(|m| {
                ...
                material_balance(&after, depth - 1).saturating_neg()
            })
            .max()
            .unwrap_or(i32::MIN)
    }
...
pub fn guess_winner(pos: &Chess) -> Color {
    pos.turn() ^ (material_balance(pos, 1) < 0)
}
